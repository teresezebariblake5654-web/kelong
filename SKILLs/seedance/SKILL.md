...
> ✨ **Node.js 版本**：此脚本使用 Node.js 实现，无需 Python 环境。通过入口脚本自动检测 Node.js 运行时（优先使用系统 node，回退到 Workhorse AI 内置运行时），Windows 和 Mac 用户都可以开箱即用。
...
**方式二：通过 Workhorse AI 启动时注入**

Workhorse AI 会自动读取系统环境变量，确保在启动 Workhorse AI 前已设置 `ARK_API_KEY`。
...
Workhorse AI 已包含 Node.js 运行时，所有必要的依赖已被自动打包。Windows 和 Mac 用户无需额外配置。
...
**路径说明**：下面的示例使用 `$SKILLS_ROOT` 环境变量来引用脚本路径。Workhorse AI 会自动设置这个变量，指向实际的 SKILLs 目录位置，因此无需手动修改路径。
...
或在 Workhorse AI 对话中说："用 Seedance 2.0 生成视频..."
