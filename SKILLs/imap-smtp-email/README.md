...
1. **Configure accounts in Workhorse AI Settings > Email.**

Workhorse AI writes `accounts.json` for multi-account setups. Existing `.env` files are still read as a legacy single-account fallback.
...
- `accounts.json` - Multi-account credentials managed by Workhorse AI Settings
