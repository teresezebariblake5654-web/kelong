# 审计：Workhorse AI Provider 配置 vs OpenClaw 官方文档
...
| Workhorse AI ProviderName | 当前 OpenClawProviderId | OpenClaw 官方 Provider ID | 状态 |
|---|---|---|---|
...
**现状**：Workhorse AI 使用 `OpenClawProviderId.Zhipu = 'zhipu'`
...
**实际影响**：由于 Workhorse AI 使用 `models.mode: 'replace'` 注入完整 provider 配置（baseUrl + api + apiKey + models），`zhipu` 会被 OpenClaw 当作**自定义 provider** 处理，基本推理功能不受影响——请求会正确发送到我们指定的 `baseUrl`。
...
**现状**：Workhorse AI 将 volcengine 的 codingPlan 模式直接用 `volcengine` provider ID 处理，没有单独的 codingPlan 路由。
...
**影响**：当用户在 Workhorse AI 中启用 volcengine 的 codingPlan 模式时，生成的配置使用 `volcengine` 作为 provider ID，而 OpenClaw 期望的是 `volcengine-plan`。这可能导致 coding 模型无法被正确路由。
...
**现状**：Workhorse AI 将 deepseek 映射到 `OpenClawProviderId.DeepSeek = 'deepseek'`
...
- `youdaozhiyun` — Workhorse AI 专有
- `stepfun` — 未列出

**注意**：由于 Workhorse AI 使用 `models.mode: 'replace'` 模式注入完整 provider 配置（含 baseUrl、api、apiKey、models），这些自定义 provider ID 不需要与 OpenClaw 内置 provider 匹配。OpenClaw 会将它们作为自定义 provider 处理。但如果 OpenClaw 对某些 provider ID 有特殊的内部逻辑（如 plugin-owned behavior），使用不匹配的 ID 会导致这些特殊逻辑不被触发。
...
OpenClaw 官方文档列出了以下 Workhorse AI 目前不支持但 OpenClaw 已支持的 provider：
...
这不是 bug，只是功能覆盖差距。但如果用户在 Workhorse AI 中配置了这些 provider，当前的 fallback 逻辑会将其路由到 `lobster` provider ID，这可能不是用户预期的行为。
