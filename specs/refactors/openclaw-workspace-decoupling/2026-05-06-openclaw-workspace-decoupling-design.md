# Workhorse AI 工作目录与 OpenClaw Workspace 解耦设计文档

## 1. 概述

### 1.1 问题/动机

在 PR #1890 之前，Workhorse AI 的 `workingDirectory` 同时承担两种职责：

1. Workhorse AI 用户选择的任务工作目录，用于桌面 Cowork 和 IM 会话的项目 cwd。
...
- 用户只想切换项目 cwd 时，Workhorse AI 需要同步 `MEMORY.md`、`IDENTITY.md` 并重写 `openclaw.json`。
...
- 用户选择的 Workhorse AI 工作目录只表达“当前任务在哪个项目目录执行”。
- OpenClaw workspace 只表达“agent 运行依赖的 profile/state 目录”，用于存放 `AGENTS.md`、`IDENTITY.md`、`USER.md`、`SOUL.md`、`MEMORY.md`、`memory/` 等文件。
- Agent 生成、读取、修改的用户项目文件必须落在用户选择的 Workhorse AI 工作目录，而不是 OpenClaw workspace。
- 主 agent 的长期记忆和个性化文件集中存放在 Workhorse AI 管理的固定 OpenClaw workspace。
- 迁移旧目录中的记忆和个性化文件，不删除源文件。
- 切换 Workhorse AI 工作目录不应导致主 agent 的记忆文件迁移，也不应仅因 cwd 变化重写 OpenClaw config。
...
| 概念 | 当前字段/函数 | 当前用途 |
|------|---------------|----------|
| Workhorse AI 任务工作目录 | `CoworkConfig.workingDirectory`、`CoworkSession.cwd` | UI 选择的任务目录，保存到 session，IM 会话也依赖它 |
...
目标语义应该是：第二行的 OpenClaw workspace 默认固定为 `{STATE_DIR}/workspace-main`，只保存 OpenClaw 依赖的 MD 文件和 `memory/`；第三行的 runtime cwd 则应由 Workhorse AI session cwd 决定。
...
- 切换 Workhorse AI 工作目录不会立即触发 OpenClaw config sync，减少 gateway reload/restart 风险。
...
同时，Workhorse AI 在 `cowork:session:start` 里虽然保存了 `taskWorkingDirectory` 并向 runtime 传入 `workspaceRoot`，但 `OpenClawRuntimeAdapter.startSession()` 没有把 `workspaceRoot` 继续传给 `runTurn()`，`chat.send` 请求也没有携带 cwd 或 workspace override。
...
`enterpriseConfigSync.ts` 仍然把外部 `openclaw.json` 的 `agents.defaults.workspace` 写回 Workhorse AI `workingDirectory`。在新的语义下，如果 `agents.defaults.workspace` 表示固定的 OpenClaw workspace，这个同步会把 Workhorse AI 任务目录改成 OpenClaw 私有目录。
...
| 新概念 | 建议命名 | 所有者 | 说明 |
|--------|----------|--------|------|
| 任务工作目录 | `taskWorkingDirectory` | Workhorse AI session | 用户选择的项目目录，是工具读写和命令执行的 cwd |
| OpenClaw workspace | `mainOpenClawWorkspace` | Workhorse AI/OpenClaw | 保存 `MEMORY.md`、`memory/`、`AGENTS.md`、`IDENTITY.md`、`USER.md`、`SOUL.md` 等 OpenClaw 依赖文件 |
| Runtime cwd | `runtimeCwd` | OpenClaw run | 当前 turn 实际使用的工具 cwd，应由 session cwd 决定 |

Workhorse AI 现有 `CoworkConfig.workingDirectory` 可以保留作为兼容字段，但代码内部新增 helper 或注释时应称为 task directory，不再称为 OpenClaw workspace。
...
```json5
{
  "agents": {
    "defaults": {
      "workspace": "/Users/me/Library/Application Support/Workhorse AI/openclaw/state/workspace-main",
...
- Native channel 会话没有显式 task cwd 时，使用 Workhorse AI 当前配置的默认 task directory，而不是 OpenClaw workspace。

### 4.3 Workhorse AI 侧调整

在 OpenClaw 支持上述能力后，Workhorse AI 应按以下方式落地：
...
4. `enterpriseConfigSync.ts`
   - 只从企业配置中的明确 task cwd 字段同步 Workhorse AI `workingDirectory`。
...
如果短期无法修改 OpenClaw API，不建议只在 Workhorse AI 侧把 `agents.defaults.workspace` 固定到 `{STATE_DIR}/workspace-main` 后直接发布。固定到 `{STATE_DIR}/workspace-main` 本身是正确目标，但它必须和 runtime cwd 分离一起生效。更安全的临时方案是：
...
3. 将主 agent 记忆和 OpenClaw 依赖文件固定目录作为 Workhorse AI 内部存储，但通过明确的 OpenClaw runtime cwd 支持后再接入 runtime。
4. 如果必须让 OpenClaw 读取固定记忆目录，应优先在 OpenClaw 增加 workspace/cwd 分离能力，而不是用提示词要求 agent 自己读绝对路径。

目标状态仍然是：OpenClaw workspace 默认在 `{STATE_DIR}/workspace-main`，用户生成文件默认在 Workhorse AI task cwd。
...
1. 在 Workhorse AI 代码中引入 `taskWorkingDirectory` 与 `mainOpenClawWorkspace` helper。
...
1. Workhorse AI 用户工作目录不再决定主 agent 记忆和 OpenClaw 依赖文件的位置。
...
7. Enterprise config 不再把 OpenClaw workspace 写回 Workhorse AI task working directory。
