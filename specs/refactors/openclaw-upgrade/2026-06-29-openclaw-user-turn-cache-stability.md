...
Workhorse AI 在 2026-06-16 将 OpenClaw 从 `v2026.4.14` 升级到
...
4. 以 `v2026.6.1` 版本专属 patch 回补上游修复，不在 Workhorse AI 业务层复制
...
boundary 的入口，避免只修 Workhorse AI 桌面 `chat.send` 后，CLI、TUI、agent 或重启
恢复入口继续产生不一致历史。
...
Workhorse AI 侧增加 patch 决策测试和强应用校验，防止 patch 文件存在但因部分冲突被
...
2. 在干净 `v2026.6.1` 上应用现有 Workhorse AI patch，建立移植基线。
3. 移植上游 `1af55bc665`，处理与现有 runtime safety patch 的单一上下文冲突。
4. 生成 `openclaw-user-turn-cache-stability.patch`。
5. 增加 patch 内容、应用结果和回归测试的 Workhorse AI 侧门禁。
6. 从干净 tag 重新应用全部 patch，运行 OpenClaw 定向测试、Workhorse AI 测试、lint、
...
| `src/main/libs/openclawPatches/userTurnCacheStability.test.ts` | Workhorse AI 侧 patch 决策与源码应用测试 |
...
timestamp 实现，并再次运行 Workhorse AI patch source/runtime 门禁。
...
| Workhorse AI OpenClaw patch 测试 | 通过；13 个文件、33 个用例，包含源码和 runtime bundle 门禁 |
...
另执行 Workhorse AI 官方完整 `npm test`：147 个文件中 145 个通过，1568 个用例中
