...
- 不在 Workhorse AI 中复制 OpenClaw cron schema、normalize、SQLite projection 和 run-log import 规则。
- 迁移失败不阻断应用启动，避免修复路径引入启动失败。
- 确保后续启动依赖 OpenClaw `.migrated` 归档机制自然跳过，不额外引入 Workhorse AI 迁移 marker。
...
使用临时 config 的原因是完整 doctor 可能修复非 cron 配置，例如 skills、plugins、gateway auth 等。cron 迁移只依赖 `cron.store` 和 state dir，因此临时 config 可以复用 OpenClaw 官方 cron migration 逻辑，同时避免改写 Workhorse AI 生成的真实配置。
