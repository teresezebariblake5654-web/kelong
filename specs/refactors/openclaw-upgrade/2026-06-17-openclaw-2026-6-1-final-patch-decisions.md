...
Workhorse AI 当前分支已将 OpenClaw pinned version 升级到 `v2026.6.1`。历史上 `v2026.4.14` 目录中仍有四个待处理 patch：
...
这些 patch 都属于运行时可靠性兜底或诊断链路防护，不能只看是否能机械套用；需要逐个确认其引入背景、对应 Workhorse AI PR/commit、OpenClaw 6.1 上游是否已覆盖，以及迁移/不迁移后的端侧验收方式。
...
- Workhorse AI PR：[#2049](https://github.com/netease-youdao/Workhorse AI/pull/2049)
- Workhorse AI commit：`5fffc70d fix(openclaw): prevent aborted tool loops from burning tokens`
- 后续补修：[#2051](https://github.com/netease-youdao/Workhorse AI/pull/2051)，commit `b46bf747 fix: re fix tool loop breaker`
...
- Workhorse AI 侧已显式写出 `tools.loopDetection.enabled=true`，但仅依赖上游 generic breaker 不能覆盖旧污染会话和跨参数 aborted 循环。
...
- Workhorse AI PR：[#1834](https://github.com/netease-youdao/Workhorse AI/pull/1834)
- Workhorse AI commit：`1ec7c2c1 fix: upgrade openclaw-weixin to 2.1.10 and add openclaw patches`
...
- Workhorse AI PR：[#2044](https://github.com/netease-youdao/Workhorse AI/pull/2044)
- Workhorse AI commit：`79988fe9 fix(openclaw): prevent subagent cleanup finalize from blocking on hook failure`
...
- Workhorse AI Windows runtime 当前优先通过 `gateway-bundle.mjs` fast path 启动，根目录没有 `subagent-registry.runtime.js`，真实文件位于 `dist/`。上游 #72731 解决 Windows 路径规范化，不解决 `gateway-bundle.mjs` 作为 base URL 时相对导入应映射到 `dist/` 的问题。
...
6. 补充 Workhorse AI 侧 patch 存在性/决策测试。
...
端侧手工验收按第 2 节每个 patch 的验收说明执行。重点观察 `C:\Users\yangwn\AppData\Roaming\Workhorse AI\openclaw\logs` 中是否仍存在 runaway tool loop、重复 subagent cleanup finalize 或 bundle runtime import 失败。
