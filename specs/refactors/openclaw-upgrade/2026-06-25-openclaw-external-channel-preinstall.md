...
这代表两者从核心 bundled extension 迁移为 official external channel plugin。Workhorse AI 的 embedded runtime 通过 `npm pack` 产物生成，因此打出的安装包不再包含 `dist/extensions/qqbot` 和 `dist/extensions/discord`。

原有 Workhorse AI 配置同步仍会写入 `plugins.entries.qqbot` 和 `channels.qqbot`。当 runtime 中没有对应插件时，OpenClaw 会报：
...
- 在构建期把 QQ/Discord 作为 Workhorse AI 随包预置的 OpenClaw official external channel plugins 安装到 `third-party-extensions`，而不是恢复到 OpenClaw 核心 `dist/extensions`。
...
Workhorse AI 不应把该动作推迟到用户启用 IM 时执行，否则首次启用会依赖网络并增加等待时间。由于现有 `openclaw.plugins` 已经表示 Workhorse AI 构建期预装到 `third-party-extensions` 的插件，因此 QQ/Discord 可以直接纳入该清单，复用既有安装、缓存、打包校验和隐藏用户插件列表的逻辑。

### 2.2 Workhorse AI 构建链路
...
Workhorse AI 已有 `ensure-openclaw-plugins.cjs`，会读取 `openclaw.plugins`，用 OpenClaw CLI 在构建期安装插件，并复制到：
...
两者仍通过 Workhorse AI 的构建期插件安装流程进入 `third-party-extensions`，不会被放回 OpenClaw core package 的 `dist/extensions`。
