# Workhorse AI Cowork 计划模式设计文档

## 1. 概述

### 1.1 问题/背景

Workhorse AI Cowork 默认以执行任务为目标：模型可以读取项目、调用工具、修改文件并验证结果。但在需求尚未明确、改动范围较大或用户希望先审阅方案时，直接执行会增加返工和误修改风险。
...
- 不实现 Codex 完整的服务端 collaboration mode 协议；当前由 Workhorse AI 在每轮 system prompt 和 runtime 约束中实现。
...
Workhorse AI 与 Codex 的实现不同：

- Workhorse AI 当前 Agent 引擎是 OpenClaw，不依赖 Codex app-server 的 collaboration mode 类型。
...
- recovery 请求使用新的 idempotency key，并绑定到同一 Workhorse AI session。
...
3. Workhorse AI `cowork.log` 中的 runtime mode、recovery 和 tool block 事件。
4. OpenClaw gateway 日志中的 run id、tool 参数、lifecycle。
5. OpenClaw session JSONL 中模型原始 assistant content。

如果 JSONL 完整而 SQLite 残缺，属于 Workhorse AI 流式/完成时序问题；如果 JSONL 本身残缺，才属于模型或上游 provider 输出问题。
...
2. 修复线上/手测问题时，先从 SQLite、Workhorse AI 日志和 OpenClaw JSONL 还原真实事件顺序。
