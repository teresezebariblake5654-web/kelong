...
Workhorse AI 当前通过 OpenClaw 提供 `browser` 和 `web_fetch` 能力，但浏览器相关配置在 Workhorse AI 中几乎没有产品化暴露。当前同步到 `openclaw.json` 的配置只有：
...
1. **全局系统代理与 browser 严格 SSRF 策略冲突**
   - Workhorse AI 的“使用系统代理”会把系统代理写入进程环境变量，并传给 OpenClaw gateway。
   ...
   - 对 Workhorse AI 本地桌面应用来说，默认拦截牺牲了常见浏览器任务成功率。

2. **`web_fetch` 被用来抓搜索结果页，容易触发 429**
   - 当前 Workhorse AI 管理配置中禁用了内置 `web_search`，agent 容易退化为用 `web_fetch` 抓 Google 搜索页。
...
- 不默认开启系统代理；当用户已开启 Workhorse AI 的“使用系统代理”时，browser 默认采用代理兼容策略，减少全局代理环境下的导航失败。
...
**Given** 用户开启了 Workhorse AI 的“使用系统代理”
...
**Then** Workhorse AI 保存清洗后的域名列表；允许列表同步到 browser SSRF policy，屏蔽列表作为 Workhorse AI 侧规则预留
...
- 不展示“启用浏览器工具”开关，Workhorse AI 始终开启 browser tool。
- 不展示“使用哪种浏览器”选择，当前版本统一使用 Workhorse AI 独立浏览器。
...
- “跟随应用全局代理”不作为独立开关暴露，避免用户误解为“默认开启系统代理”。
  - 在推荐模式下，browser / web_fetch 默认跟随 Workhorse AI 现有的“使用系统代理”总开关。
...
- 提供“屏蔽的域名”列表。
  - OpenClaw 当前 browser 配置没有对应字段，第一阶段可仅作为 Workhorse AI 侧配置预留。
  - 第二阶段在 OpenClaw browser 导航 guard 或 Workhorse AI tool 调用前增加拦截。
- 屏蔽规则优先级高于允许规则。
...
- 屏蔽的域名，第一阶段作为 Workhorse AI 侧配置预留。
...
Workhorse AI 当前版本不支持 sandbox browser，因此设置页不提供 sandbox / host / node 的目标选择。
...
- 该策略先通过 Workhorse AI 托管的 AGENTS 指令下发；如果后续仍出现模型误传 `target="sandbox"`，再推动 OpenClaw browser 插件增加 Workhorse AI 侧的 target override 或禁用 sandbox target。
...
| 高级设置 | 屏蔽的域名 | 空 | 预留为 Workhorse AI 侧导航拦截规则 |
...
- 使用 `browser` 时必须显式设置 `target="host"`，因为 Workhorse AI 当前不支持 sandbox browser。
...
1. 新增 Workhorse AI `browserWebAccess` 配置模型和默认值。
2. 新增“浏览器”设置页。
...
2. Workhorse AI 根据全局代理和默认配置写入该配置，不在浏览器设置页展示。
