# Workhorse AI Cowork 会话分叉设计文档

## 1. 概述

### 1.1 问题/背景

Workhorse AI Cowork 当前已经具备完整的本地会话、消息持久化、OpenClaw 工具调用和工作目录执行能力。用户在一个会话中经常会遇到以下需求：
...
- 不把 OpenClaw 原始 transcript 文件复制为新 Workhorse AI session 的运行态来源；第一版只读取 checkpoint 摘要并显式桥接。
- 不在第一版实现复杂的分叉树可视化。
...
因此，Workhorse AI 第一版不实现非 Git 文件快照。对于文件类任务，推荐用户使用 Git repo + worktree。
...
结合 OpenClaw 源码和 gateway schema，Workhorse AI 不能把“UI 上的压缩分隔符”和“可用于分叉续写的压缩摘要”混为一谈：
...
- gateway 还提供 `sessions.compaction.branch` / `sessions.compaction.restore`，但它们操作 OpenClaw transcript/session store，不等同于 Workhorse AI 本地 Cowork session 分叉，第一版不直接采用。
- `chat.history` 面向普通聊天历史窗口，且 Workhorse AI 当前的 `extractGatewayHistoryEntry()` 只接受 `user` / `assistant` / `system`，不会可靠保留 `compactionSummary`。

因此，Workhorse AI 分叉压缩会话时应优先从 OpenClaw checkpoint metadata 读取 `summary`，而不是从聊天气泡、`COMPACTION` 分隔符或普通 `chat.history` 文本中推断。
...
**When** 用户点击“分叉会话”  
**Then** Workhorse AI 创建一个新的 Cowork session
...
**When** 用户选择“在新工作区分叉”  
**Then** Workhorse AI 创建一个 Git worktree
...
**When** 用户打开分叉菜单  
**Then** Workhorse AI 允许“分叉会话”

**And** Workhorse AI 禁用“在新工作区分叉”
...
**When** 用户分叉会话或工作区  
**Then** Workhorse AI 提示命令造成的环境变化不会随分叉回滚或复制
...
**When** 用户选择“从这里分叉”  
**Then** Workhorse AI 创建新的 session
...
**And** Workhorse AI 提示当前文件和工作树状态不会回滚到该消息时刻
...
**When** 用户发起分叉  
**Then** Workhorse AI 禁止直接分叉或提示先停止当前任务
...
Workhorse AI 需要为 session 计算工具副作用级别：
...
分叉 session 必须使用新的 Workhorse AI session id。
...
```text
[Compacted context from the source Workhorse AI conversation]
...
1. 使用源 Workhorse AI session id 和 agent id 解析 OpenClaw managed session key。
...
这些状态不属于 fork 的可复制范围。Workhorse AI 只提示，不承诺回滚。
...
**Goal:** Add Codex-style local Cowork session fork support to Workhorse AI, while keeping Git worktree workspace isolation as a deferred Phase 2 design.

**Architecture:** Implement fork as a main-process CoworkStore operation that creates a new session and copies persisted messages. For compacted OpenClaw sessions, read checkpoint summary metadata through the gateway and persist it as hidden bridge context in the fork. Keep OpenClaw session isolation by using the new Workhorse AI session id and existing session-key derivation. Git worktree isolation is deferred.
...
- 消息级早期分叉如何精确判断 checkpoint 是否早于分叉点？是否需要在 Workhorse AI 本地记录 checkpoint 与 message sequence 的映射？
