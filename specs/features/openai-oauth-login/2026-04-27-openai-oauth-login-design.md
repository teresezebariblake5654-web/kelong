...
Workhorse AI 原有 OpenAI 接入只支持自定义 API Key。新增 ChatGPT OAuth 登录后，需要满足以下要求：
...
|---|---|---|
| Workhorse AI provider | `openai` | `openai` |
...
3. **OpenClaw 使用独立 Codex home。** `CODEX_HOME` 指向 Workhorse AI app data 下的 `codex` 目录，避免影响系统 Codex CLI。
...
当 Workhorse AI 设置中启用系统代理，并且目标 base URL 不是 loopback 地址时，OpenClaw provider 写入：
...
因此 OpenClaw/pi-ai 会从 Workhorse AI 管理的 `CODEX_HOME/auth.json` 读取 ChatGPT OAuth token。
...
2. 选择 `ChatGPT 登录`
3. 完成浏览器 OAuth 登录
4. 返回 Workhorse AI，设置页应立即显示已登录状态
...
| 系统 Codex CLI 已登录 | Workhorse AI 不覆盖 `~/.codex/auth.json` |
...
2. OAuth token refresh 依赖 OpenClaw/pi-ai 读取 `auth.json` 后的 provider 行为；Workhorse AI 当前只负责初次 token exchange 和本地持久化。
