...
配置值存储在 `openclaw.json` 的 `plugins.entries[pluginId].config` 中。OpenClaw 自带的 web UI（`/automation` → Plugins tab）能够解析这些字段并动态渲染配置表单。Workhorse AI 需要在自身的插件管理页面中提供同等能力。

### 1.2 目标

- 用户可在 Workhorse AI 的插件管理页面中查看并编辑插件的自定义配置项
