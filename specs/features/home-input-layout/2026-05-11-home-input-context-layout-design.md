...
日期：2026-05-11  
分类：功能  
参考：用户提供的 Codex 输入框参考图与当前 Workhorse AI 首页截图  
...
当前 Workhorse AI 首页里，模型选择器位于页面顶部 header 左侧；文件夹选择器位于大输入框内部左下；当前选中的 Agent 没有在输入区附近显式展示。用户希望首页输入区更接近参考图的信息层级：
...
**Given** 用户打开 Workhorse AI 首页，当前没有会话详情  
**When** 用户查看大输入框底部工具栏  
