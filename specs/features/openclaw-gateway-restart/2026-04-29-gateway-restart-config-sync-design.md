...
用户正常使用 Workhorse AI Cowork 时，未主动修改配置，但 OpenClaw gateway 在后台突然收到 `SIGTERM` 并重启。日志中同时出现了插件配置 warning：

```text
[Auth:getModels] Fetching: https://api.bx-aigc.com/api/models/available
...
**`package.json` 中的 OpenClaw plugin id 是 Workhorse AI 用来安装/定位插件包的 package/directory id；OpenClaw runtime 校验 `plugins.entries` 时使用的是插件 `openclaw.plugin.json` 里的 manifest id。两者不能混用。**
...
因此，`package.json` 里有 `clawemail-email` 和 `openclaw-nim-channel` 只能说明 Workhorse AI 会安装这两个插件包，不代表 `openclaw.json` 里可以使用同名 entry。
...
2. **config entry 使用 runtime id。** 所有 `plugins.entries` key 都必须是 OpenClaw manifest id，而不是 npm 包名、安装目录名或 Workhorse AI package id。
3. **插件不存在就不写入。** `qwen-portal-auth` 这类 runtime 未安装插件不能因为 provider URL 命中而无条件写入。
4. **保留 runtime 注入字段。** 继续保留 gateway/runtime 自动注入的 `plugins` 和 `gateway` 字段，避免因为 Workhorse AI 重写配置导致新的 diff loop。
...
```text
~/Library/Application Support/Workhorse AI/openclaw/state/openclaw.json
```
...
```text
~/Library/Application Support/Workhorse AI/openclaw/state/openclaw.json.bak-2026-04-28T15-46-16-321Z
```
...
剩余风险主要来自 OpenClaw 未来版本可能再次调整插件 manifest id。该风险由 manifest 扫描机制承接，只要插件包内 `openclaw.plugin.json` 正确，Workhorse AI 不需要硬编码新 id。
