# Workhorse AI Cowork Steer 设计文档

## 1. 概述

### 1.1 问题/背景

Workhorse AI Cowork 当前把一次用户提交视为一个完整 turn。Agent 运行期间，
...
Workhorse AI 需要在 Cowork 中实现类似能力，并与当前 OpenClaw runtime、Cowork
...
- 不实现 Codex App 前端的完整交互复刻；Workhorse AI 采用自身输入框布局和视觉语言。
...
- 不在第一版修改 OpenClaw 源码，除非确认当前 Workhorse AI 侧没有可用集成点。
...
{ steeringMode: "all" })` 的 RPC，Workhorse AI 应在 adapter 内封装为：
...
截图里的同轮 steer。Workhorse AI 的 `Steer` 主路径必须保持当前 turn 继续运行，只
...
3. Workhorse AI 本地 streaming state fallback。

没有 reliable active turn id 时，不直接调用 steer；先进入 pending，并等待下一个
runtime event 补齐。

### 6.6 当前 Workhorse AI 对接点与异常点
...
- placeholder：`Ask for follow-up changes` 的 Workhorse AI 文案可为
  “补充当前任务的调整要求”。
