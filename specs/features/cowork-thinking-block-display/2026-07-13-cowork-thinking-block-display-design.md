...
Workhorse AI 在 2026 年 5 月 20 日合入 PR #2019（`feat/model-custom-params`）。其中提交
`d989c082` 为 thinking 内容建立了独立消息生命周期，`e5dc5396` 与 `9df3f2ae` 补充了
`beforeMessageId` 链路，使单个 thinking block 可以显示在普通 assistant 回复之前。

当时 OpenClaw 提供给 Workhorse AI 的实时事件不能稳定表达多轮工具调用之间的 thinking
...
支持的模型还可以产生 `agent stream=thinking` 增量事件。因此 Workhorse AI 可以采用
...
**Then** Workhorse AI 从 history 提取当前用户轮次的 thinking，并按 assistant/tool 边界插入
到相应工具或最终回复之前。
...
对 Workhorse AI 管理的 OpenClaw 会话应用模型覆盖时，同时写入
`reasoningLevel: 'stream'`。不支持 thinking 的模型可以忽略该设置，Workhorse AI 必须继续支持
history-only 路径。
