...
Add a configurable send-message shortcut to Workhorse AI's cowork chat input. Users can choose from four fixed candidates and switch via a dropdown next to the send button or via the Settings > Shortcuts panel. Both entry points stay in sync through the existing `app_config.shortcuts` store.
