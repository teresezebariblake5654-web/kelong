...
Add a simple user-facing setting in Workhorse AI to control how long a conversation keeps its existing context before automatically starting a new session.

The goal is to expose one clear product concept while keeping OpenClaw's lower-level session policy details internal.

## Problem

Workhorse AI currently writes only `session.dmScope` into runtime `openclaw.json`, while QClaw also configures `session.reset` and `session.maintenance`.

This creates two product issues:

- Users cannot control when old context should roll over into a new session.
- Session lifecycle behavior is not explicit in Workhorse AI and depends more on OpenClaw defaults than intended.
...
- Keeping these internal makes the UI easier to understand and gives Workhorse AI freedom to tune defaults later.
...
**Goal:** Add a simple global `会话保持时长` setting in Workhorse AI, persist it locally, and always generate explicit OpenClaw `session.reset` and `session.maintenance` config from that value.
...
openClawSessionKeepAliveHint: 'Continue chatting within this period to keep the same context. After that, Workhorse AI starts a new session automatically. Longer durations improve continuity but can also carry older context forward.',
...
- [ ] **Step 4: Render the new control in the existing Workhorse AI settings section**
...
- Change it to `1年`, save settings, and confirm `/Users/wulei/Library/Application Support/Workhorse AI/openclaw/state/openclaw.json` includes:
