# Workhorse AI Cowork 选中文本添加到对话设计文档

## 1. 概述

### 1.1 问题/背景

Workhorse AI Cowork 当前支持用户直接输入问题、添加文件和图片附件、选择 Skill / Kit，并通过 OpenClaw Gateway 发起对话。在阅读 assistant 回复时，用户经常需要针对其中一小段内容继续追问，例如：
...
Workhorse AI 可以参考这一交互，但必须适配现有 OpenClaw 接入方式。

### 1.2 OpenClaw 能力确认

Workhorse AI 当前固定使用 OpenClaw `v2026.4.14`，并通过 Gateway RPC `chat.send` 发起 Cowork turn。
...
```ts
{
  ...
  // Workhorse AI patch 额外增加 cwd?: string
}
```
...
因此，Workhorse AI 不能直接向 `chat.send` 增加以下自定义字段：
...
- `chat.history` 用于读取展示归一化后的历史，不提供 Workhorse AI 自定义 UI 引用结构。
...
1. Workhorse AI SQLite 保存结构化文本片段，服务于 UI 恢复、展开和来源定位。
...
5. 支持在同一次追问中添加多个文本片段。
6. 支持查看和移除已选片段。
7. 发送消息时，把结构化片段保存到 Workhorse AI SQLite 的 user message metadata。
8. 发送给 OpenClaw 时，把片段序列化为 `chat.send.message` 中的上下文区块。
9. 历史 user message 中展示“已选文本片段”标记，并支持展开查看快照。
10. 保持 user message 主体简洁，不在 Workhorse AI 对话气泡中重复渲染发送给 OpenClaw 的包装文本。
...
- 不在 OpenClaw transcript 中保存 Workhorse AI 的 `sourceMessageId` 或 DOM offset。
...
- Workhorse AI UI 保持简洁。
- 模型能够看到片段原文。
- OpenClaw 后续 turn 能从 transcript 中继续理解引用上下文。
- Workhorse AI 重新打开历史会话后能恢复片段 chip 和来源关系。

### 2.2 Workhorse AI 与 OpenClaw 的历史差异

同一个用户 turn 在两侧的持久化形态不同：

| 存储位置 | `content` | metadata | 用途 |
|---------|-----------|----------|------|
| Workhorse AI SQLite | 用户输入的原始请求，例如 `这个是什么意思？` | 结构化 `selectedTextSnippets` | 本地 UI、历史恢复、来源定位 |
| OpenClaw transcript | 片段区块 + 当前请求 | OpenClaw 自身字段 | 模型上下文、OpenClaw 后续历史 |

这是有意设计，不要求两侧 `content` 字符串完全一致。

Workhorse AI 当前 managed Cowork session 已经采用类似边界：
...
**Then** Workhorse AI 保存用户问题和结构化文本片段
...
**When** 用户点击 `添加到对话`
**Then** Workhorse AI 拒绝添加并展示 i18n toast
...
| 字段 | 说明 |
|------|------|
| `id` | Workhorse AI 生成的片段 id，用于草稿列表 key 和删除 |
...
3. 把片段保存到 Workhorse AI user message metadata。
...
- 不把 `sourceMessageId` 发给模型，避免注入无意义的 Workhorse AI 内部 id。
- 片段区块只在本次 user turn 中追加一次。
- 不把序列化后的包装文本回写到 Workhorse AI user message `content`。
...
第一版只对 Workhorse AI managed Cowork session 开放文本选择入口。

原因：

- managed Cowork session 中，Workhorse AI SQLite 是 UI 展示的主要来源。
- OpenClaw transcript 保存序列化后的包装文本，用于模型上下文。
- IM / channel session 会通过 `chat.history` reconcile 覆盖本地 user / assistant 消息。
- `chat.history` 只能恢复包装后的 user message 字符串，不能恢复 Workhorse AI 专用 `sourceMessageId` 与片段列表。
...
- Workhorse AI 的版本 patch 只增加 `cwd?: string`。
...
5. Workhorse AI 对应版本 patch 中的 `chat.send` 改动
...
```text
1. Workhorse AI system instructions（如有变化）
...
2. Workhorse AI patch 只新增 `cwd?: string`，没有文本引用协议。
...
| OpenClaw `chat.history` 截断长消息 | Workhorse AI managed session 仍从本地 SQLite 展示干净 user message |
...
- Workhorse AI SQLite user message `content` 仍为原始用户请求。
...
5. 查看 Workhorse AI SQLite / UI，确认保存和展示的是原始问题 + snippets metadata。
...
- [ ] Workhorse AI SQLite user message `content` 只保存用户原始请求。
- [ ] Workhorse AI SQLite user message metadata 保存结构化 `selectedTextSnippets`。
...
5. OpenClaw runtime 仍使用 `v2026.4.14` 与现有 Workhorse AI patch，不需要额外 rebuild 语义变化。
...
- `cwd` 继续由 Workhorse AI 现有 patch 支持。
- 图片继续进入 `attachments`。
- 不新增 `metadata`。
