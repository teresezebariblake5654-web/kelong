...
7. `OpenClawRuntimeAdapter.buildOutboundPrompt()` 把最终 system prompt 包装成 `[Workhorse AI system instructions]` 后放入 `chat.send.message`。

关键问题在第 3 步：`skill.prompt` 来自 `SKILL.md` 正文，用户选择越多，注入内容越大。

### 4.2 当前 OpenClaw 原生 Skill 配置

`OpenClawConfigSync` 已经把 Workhorse AI Skill 目录同步给 OpenClaw：
...
Main process 继续追加 scheduled-task prompt，OpenClaw adapter 继续通过 `[Workhorse AI system instructions]` 前缀发送，不需要修改 IPC 契约。
