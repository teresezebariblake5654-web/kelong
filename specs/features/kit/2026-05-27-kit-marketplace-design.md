...
Workhorse AI 已有 Skill（技能）体系，用户可以为 Agent 加载单个 Skill 来扩展能力。但在实际使用中，某些场景需要多个 Skill 协同工作才能完成较复杂的任务（如"翻译专家"需要术语库 + 翻译引擎 + 校对三个 Skill 配合）。单独安装和管理多个 Skill 的体验较碎片化，用户难以发现"哪些 Skill 组合在一起效果最好"。
...
数据源 URL:
- test: `https://api.bx-aigc.com/openapi/get/luna/hardware/lobsterai/test/kit-store`
- prod: `https://api.bx-aigc.com/openapi/get/luna/hardware/lobsterai/prod/kit-store`
