...
Workhorse AI 设置页中的“邮箱”当前对应内置 Skill
...
因此本需求不按“同步上游版本”处理，而是作为 Workhorse AI 内置邮箱 Skill 的自主
演进。
...
- 说明配置由 Workhorse AI 设置页管理，不要求用户编辑配置文件。
...
OpenClaw 仍通过现有 Skill 机制读取 `SKILL.md` 和执行脚本。Workhorse AI 不需要新增
OpenClaw channel 或 runtime gateway 配置。

本需求只改变：

- Workhorse AI 设置页如何管理该 Skill 的配置。
- Skill 脚本如何解析多账号配置。
