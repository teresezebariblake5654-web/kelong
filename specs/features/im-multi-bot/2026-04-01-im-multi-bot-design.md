...
Workhorse AI 原有的 IM 接入架构中，钉钉、飞书、QQ 三个平台均为**单机器人**模式 — 每个平台仅支持配置一组凭证（如一个 `clientId` + `clientSecret`）。本次变更将这三个平台升级为**多实例（Multi-Instance）**模式，允许用户为同一平台配置多个独立的机器人实例，每个实例拥有独立的凭证、策略和状态。
