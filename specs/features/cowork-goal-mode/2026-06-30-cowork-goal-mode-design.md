...
Workhorse AI currently treats Cowork sessions as ordinary chat sessions and does
...
Add a Workhorse AI client goal mode that mirrors OpenClaw's session-goal model:
...
3. Use the existing Cowork/OpenClaw session key mapping instead of creating a
   second Workhorse AI goal table.
...
6. Keep the model-side goal tools available through OpenClaw; Workhorse AI should
...
- Workhorse AI will not create an independent goal persistence layer while
  OpenClaw owns goal state.
- Workhorse AI does not expose `/goal` command text as normal user-visible chat
...
OpenClaw remains authoritative. Workhorse AI normalizes the `goal` field from
...
For existing sessions, Workhorse AI sends goal mutations through the OpenClaw
