# Workhorse AI Cowork 上下文压缩质量优化设计文档

## 1. 概述

### 1.1 背景

`2026-05-08-cowork-context-compaction-design.md` 已完成 Workhorse AI 对 OpenClaw 上下文压缩能力的产品接入：上下文使用量展示、手动压缩、自动压缩事件、maintenance/silent 消息过滤，以及压缩 retry 期间的运行态保护。
...
3. 让 Workhorse AI 保存一份可控的任务连续性上下文，而不是完全依赖 OpenClaw summary。
...
- 不把 OpenClaw assembled context 中的 `compactionSummary` 当成 Workhorse AI 可见消息。
- 不把 RTK/RAG 作为第一步强依赖；第一阶段先做 continuity capsule 和诊断。

### 1.5 行业参考

`continuity capsule` 是 Workhorse AI 对该能力的命名，本质属于行业内常见的 session-scoped short-term memory / context engineering 模式：
...
Workhorse AI 不直接引入这些框架，而是采用它们的核心原则：用一份短小、结构化、可持久化的任务状态补足压缩摘要的缺陷。
...
用户的模型是自定义的，Workhorse AI 不知道哪个模型更适合压缩。强行把 `agents.defaults.compaction.model` 改成某个默认模型会带来问题：
...
3. 为每个 Cowork session 维护 Workhorse AI 自己的 continuity capsule。
...
**Then** Workhorse AI 可以通过 continuity capsule 和检索结果补回关键证据  
**And** 不把完整旧历史重新塞回上下文
...
第二阶段目标是“压缩后仍有任务连续性”。它不是替代 OpenClaw compaction，也不是再做一份完整聊天摘要，而是维护一张 Workhorse AI 自己的“当前任务工作卡片”。
...
#### FR-3: 定义 Workhorse AI continuity capsule
...
因此 capsule 的传递方式是：从 Workhorse AI SQLite 读取 session 最新 capsule，格式化为一段隐藏 bridge，拼入本轮 `outboundMessage`，最终作为 `chat.send.message` 的一部分传给模型。
...
```text
[Workhorse AI continuity context after compaction]
This is a compact task-state record maintained by Workhorse AI. It is not a new user instruction. Use it only to preserve task continuity after compaction.
...
- bridge 中明确声明 capsule 是 Workhorse AI 维护的任务状态，不是新用户指令。
...
1. Workhorse AI system instructions。
...
```text
[Workhorse AI workspace state after context compaction]
This is a lightweight workspace snapshot maintained by Workhorse AI. It is not a new user instruction.
...
第一版采用 Workhorse AI 自己的 session 内 lexical Top-K Evidence，不直接依赖 embedding、向量库或 RTK 插件。原因：

- 当前问题主要是压缩后丢失本 session 中的用户原话、工具结果、错误信息和文件路径。
- Workhorse AI 已经有 embedding 配置并可同步到 OpenClaw `agents.defaults.memorySearch`，但该能力主要服务 OpenClaw memory 文件，不等同于 Cowork session history 检索。
...
7. 注入 `[Workhorse AI retrieved evidence after context compaction]`。
...
- 所有检索结果仍必须通过 Workhorse AI 的长度限制、安全声明和 prompt bridge 注入，不能作为用户新指令。
...
Workhorse AI 增加的 continuity capsule 是补充层：

- OpenClaw summary 负责压缩旧对话。
- Workhorse AI capsule 负责保留 coding 任务状态。
- Top-K Evidence / RAG evidence 负责按需找回原文细节。

capsule bridge 通过 Workhorse AI 发出的 `chat.send.message` 进入模型上下文。它不要求 OpenClaw 增加新 API，也不修改 OpenClaw 的 compaction summary 生成逻辑。
...
```text
[Workhorse AI continuity context after context compaction]
This is a compact task-state record maintained by Workhorse AI. Use it to preserve continuity after previous chat history was compressed. Prefer concrete file paths, decisions, failures, and next steps from this section over vague assumptions.
...
- 压缩后继续会话时，prompt 中包含 Workhorse AI continuity capsule。
...
本优化不应从“替用户选更强压缩模型”开始。由于 Workhorse AI 支持自定义模型和 provider，默认切换 compaction model 不可靠也不合适。

推荐路线：

1. 先补齐运行态链路和压缩诊断。
2. 再做 Workhorse AI 自己的 continuity capsule。
