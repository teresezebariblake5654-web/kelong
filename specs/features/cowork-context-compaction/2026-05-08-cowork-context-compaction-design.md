# Workhorse AI Cowork 上下文压缩接入设计文档

## 1. 概述

### 1.1 问题/背景

Workhorse AI Cowork 当前已经基于 OpenClaw 运行对话与工具调用。OpenClaw 本身具备上下文自动压缩能力：当会话上下文接近或超过模型上下文窗口时，OpenClaw 可以压缩旧历史，保留摘要和最近消息，并继续重试原请求。

但 Workhorse AI 当前对这个能力的产品接入还不完整：

- 对话界面没有直观展示当前上下文 token 使用量、上下文窗口大小和百分比。
- 用户无法在 Workhorse AI 中主动触发 OpenClaw 压缩。
- OpenClaw 自动压缩发生后，Workhorse AI 对话流中没有轻量提示，用户可能只感知到“等待变久”。
...
- OpenClaw 的 `Pre-compaction memory flush`、`NO_REPLY/no_reply`、memory daily file read/write 等内部维护消息可能通过 history 或 stream 暴露给 Workhorse AI；这些内容不应作为正式聊天内容展示。
- OpenClaw Web UI 中的 `COMPACTION` 分隔符、assembled context 中的 `compactionSummary`、session checkpoint 是三个不同层级，Workhorse AI 不能混为一谈。
- `reconcileWithHistory()` 可能替换 Workhorse AI 本地 assistant message id，导致后续 usage/meta 回填写到旧 id，从而让 assistant 下方 token/meta 信息消失。
...
8. 避免在 OpenClaw 自动压缩、重试或 failover 过程中被 Workhorse AI 过早 abort。
...
**Given** 用户看到上下文指示器  
**When** 用户点击指示器
**Then** Workhorse AI 调用 OpenClaw 的压缩能力
...
**Then** Workhorse AI 在对话流中展示 `OpenClaw 已自动压缩上下文。`
...
**Then** Workhorse AI 不应在 2 秒后立即 `chat.abort`
...
**Then** Workhorse AI 应明确提示当前会话仍在运行
...
**Then** Workhorse AI 不展示该 user prompt

**And** 如果 assistant 回复为纯 `NO_REPLY/no_reply`，Workhorse AI 不展示该 assistant 消息

**And** 如果本轮确认发生了 memory daily file read/write，Workhorse AI 保持 session running 并展示 `正在整理上下文...`
...
**Then** Workhorse AI 不插入“上下文已自动压缩”的 checkpoint 提示

**And** Workhorse AI 可继续展示 context usage 已超出窗口的 danger 状态
...
**Then** Workhorse AI 应校验 preferred assistant message id 是否仍存在
...
**Then** Workhorse AI 主动向 OpenClaw 同步一次 context usage

**And** 如果 OpenClaw 返回完整 token/percent 数据，输入框附近展示上下文圆环

**And** 如果 OpenClaw 暂时无法返回完整 usage，Workhorse AI 不展示圆环，也不展示“不可用”tooltip
...
Workhorse AI 需要为 Cowork session 维护上下文使用量状态：
...
3. Workhorse AI 本地 `models.json` 中的 `contextWindow` 作为分母兜底。
...
当 OpenClaw 自动压缩发生后，Workhorse AI 应在对话流中插入轻量系统提示：
...
当 OpenClaw session 仍在 running 时，Workhorse AI 需要避免用户误判任务结束。
...
- 用户可能配置大量自定义模型，Workhorse AI 无法精确知道所有模型窗口。
- 贸然改小默认窗口会影响常规模型体验，导致过早压缩。

后续如果服务端能提供 Workhorse AI server 模型 metadata，可以优先让可控模型写准 `contextWindow`，但不属于本功能第一版。
...
Workhorse AI 需要过滤以下 OpenClaw 内部消息：
...
当满足以下条件时，Workhorse AI 应进入 context maintenance running state：
...
当前 Workhorse AI 第一版使用专用 `sessions.compact` RPC，不通过发送 `/compact` 文本模拟用户消息，避免污染聊天历史。
...
7. OpenClaw runtime adapter 解析 Workhorse AI session 对应的 OpenClaw `sessionKey`。
...
- OpenClaw `sessions.compact` 在 manual compact 且未传 `maxLines` 时会先调用 `interruptSessionRunIfActive(...)`，所以 Workhorse AI 不允许在任务仍运行时主动压缩，避免打断正在执行的用户任务。
...
- 2026-05-09 实测确认：memory flush 或内部计数变化可能让 `compactionCount` 变化，但 OpenClaw Sessions 页面仍显示 `COMPACTION none`；因此 Workhorse AI 已改为只使用 `compactionCheckpointCount` 触发自动压缩提示。
...
2026-05-09 实测中，OpenClaw Chat UI 显示 `COMPACTION` 分隔符，并且 context usage 达到约 `80.3k / 60k`；但 Sessions 页面 `COMPACTION none`，`compactionCheckpointCount` 仍为空。因此 Workhorse AI 不应插入“上下文已自动压缩”提示。
...
- 如果 OpenClaw Chat UI 显示 `COMPACTION` 分隔符，但 Sessions 页面仍是 `COMPACTION none`，Workhorse AI 不插入“上下文已自动压缩”提示。
...
- OpenClaw Chat UI 显示 `COMPACTION` 但 Sessions 页面 checkpoint 为 none 时，Workhorse AI 不提示“已自动压缩”。
...
- 不对未知模型强行改小窗口。
- context exceeded 后依赖 OpenClaw overflow recovery 和 Workhorse AI 延迟 abort 修复。

开发测试策略：

- 测试期间曾临时将 Workhorse AI 同步给 OpenClaw `models.json` 的 `contextWindow` override 设为 `60_000`，用于更容易触发 memory flush / pre-compaction / overflow 相关路径。
- 该测试 override 已在发布前移除，生产逻辑恢复为：仅当 provider 本身声明 `modelDefaults.contextWindow` 时同步该值；未知模型不强行写入窗口，继续由 OpenClaw 使用默认 `200k` 兜底。
- 测试覆盖值仅影响 Workhorse AI 同步给 OpenClaw 的模型配置；如果 OpenClaw provider catalog 对某模型有更高优先级，最终分母仍应以 OpenClaw `sessions.list` 返回的 `contextTokens` 为准。
...
6. 已确认：OpenClaw gateway 还暴露 `sessions.compaction.branch` / `sessions.compaction.restore`，但它们操作 OpenClaw transcript/session store，不等同于 Workhorse AI Cowork 会话分叉。
7. 已确认：OpenClaw `compactionSummary` 是 assembled/model-visible context 的角色，不应与 UI `COMPACTION` 分隔符或 Workhorse AI 的轻量系统提示混为一谈。
8. 已处理：自动压缩期间可能与 `chat.final` / retry stream 交错，Workhorse AI 通过 800ms deferred completion 和 stream cancel 处理。
...
7. Workhorse AI 端在继续执行期间缺少稳定的 running/loading 表达，用户容易误以为任务已结束。
...
16. 进一步代码排查显示，初始 loading 丢失发生在用户再次发送之前：`19:54:03` Workhorse AI 收到 `chat.final` 并 emit `complete`，renderer 因此把当前会话置为 completed，`isStreaming` 变为 false。
...
21. 因此 loading 丢失的最可能根因是“OpenClaw 在 context overflow/auto-compaction/retry 前后出现 chat.final/后续 stream 的组合，Workhorse AI 过早 complete 了 renderer 状态，而后续 active turn 重建没有同步 running 状态给 renderer”。
...
```text
/Users/zhiqiangliu/Library/Application Support/Workhorse AI/openclaw/state/agents/main/sessions/b1a58bee-1aee-41fd-bfe1-490f3c186667.checkpoint.70b36695-0e4f-4567-a7e3-c29f0d8181bc.jsonl
```
...
- OpenClaw 对话 UI 中可能只展示轻量 `COMPACTION` 分隔符，或主要在 Sessions 列表和 `Show checkpoints` 中体现；Workhorse AI 不能依赖普通聊天消息来判断压缩发生。
- Workhorse AI 需要主动对接 compaction metadata，才能在自身对话流中展示用户可理解的“已自动压缩上下文”提示。
...
```text
Workhorse AI sessionId: eedbc56e-9d03-4281-a5b6-199c1513865d
...
3. `19:54:03` OpenClaw 对同一 run 发送 `chat.final`，Workhorse AI 收到后 emit `complete`，renderer 将当前会话置为 completed，`isStreaming` 变为 false。
...
6. `19:58:35` Workhorse AI main 侧在后续 OpenClaw stream 到来时重新创建 ActiveTurn，并将 main store 中 session 状态改回 `running`。
...
12. `20:36:11` Workhorse AI 通过 lifecycle end fallback 完成 missed `chat.final` 的最终同步。
13. `20:36:12` Workhorse AI usage sync 完成，日志显示 `in=361 out=302 ctx=0% cacheRead=138624 agent=main`。
...
本次用户感知到的“任务还在跑，但 Workhorse AI loading 丢失、输入框恢复可发送”不是 OpenClaw 任务停止，也不是 `Session ... is still running.` 提示本身导致的。

更准确的根因是：

1. OpenClaw 在 context overflow / auto-compaction / retry 前后出现了 `chat.final` 后继续同一 run stream 的组合。
2. Workhorse AI renderer 收到第一次 `complete` 后提前把 UI 状态改成 idle。
...
如果仍保持旧逻辑：收到 `lifecycle phase=error` 后 2 秒就发送 `chat.abort`，那么在 OpenClaw 刚检测到 context overflow 并准备自动压缩/retry 时，Workhorse AI 很可能会提前 abort gateway run。
...
- 测试时本地曾将 Workhorse AI 同步给 OpenClaw 的 context window override 设为 `60_000`，用于更快复现相关路径；该 override 已在发布前移除。
- 测试 session key：
...
- Workhorse AI 需要在 confirmed maintenance 期间保持 running，并显示“正在整理上下文...”，否则用户会误以为任务停止。
...
- Workhorse AI 不应在该情况下插入“上下文已自动压缩”提示。
...
测试发现 Workhorse AI 端 assistant 回复下方 meta 信息不展示，而 OpenClaw 端可以看到类似：

```text
Workhorse AI 13:47 ↑80.4k ↓391 100% ctx qwen3.6-plus
```
