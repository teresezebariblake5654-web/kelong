...
模型提供商会不断引入新的 API 参数（如 DeepSeek 的 `reasoning_effort` 新增 `smart` 选项、Anthropic 的 `thinking` 配置等）。这些参数通常是模型/提供商特有的，Workhorse AI 无法为每个新参数都预设 UI 字段。
...
本功能通过 Workhorse AI 侧 patch OpenClaw runtime，增加 `extra_body` 通用透传机制，将 `params.extra_body` 中的字段直接合并到 API 请求 body，绕过白名单限制。
