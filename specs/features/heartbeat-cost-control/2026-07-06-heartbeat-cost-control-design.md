...
有用户反馈"什么也没做,积分也在持续消耗"。排查确认消耗来自 OpenClaw 心跳机制:Workhorse AI 在
...
1. **旧版模板缺陷**:老版本 runtime 初始化 workspace 时写入的 `HEARTBEAT.md` 模板含普通正文
   ...
   OpenClaw 自带 `doctor-heartbeat-template-repair` 就是修这个的,但 Workhorse AI 从未替用户
   执行过。
2. **agent 自发写入**:Workhorse AI 托管的 AGENTS.md 段落明确鼓励
...
- 不修改 OpenClaw runtime、不新增版本补丁(全部用现有配置字段与 Workhorse AI 侧文件操作实现)。
...
Workhorse AI 侧修复采用同一算法与同一目标模板,保证与 runtime 未来行为一致。
...
Workhorse AI 托管区(每次同步重写)。runtime 模板含 `## 💓 Heartbeats - Be Proactive!` 段,
