...
Workhorse AI 基于 OpenClaw 运行 Agent。OpenClaw 已支持通过 `sessions_spawn.agentId` 将子任务委派给另一个已配置 Agent，但仅配置 `subagents.allowAgents` 不会让模型自动把 `taskName` 当成目标 Agent。
...
本功能目标是让用户在 Agent 页面配置“当前 Agent 可委派哪些其它 Agent”，并由 Workhorse AI 生成符合 OpenClaw 语义的配置，使 main Agent 可以通过自然语言协调预配置的协作 Agent。
...
**Then** Workhorse AI 保存用户选择，并在 OpenClaw 配置中为 main 输出可委派目标。
...
同步 OpenClaw 配置时，Workhorse AI 基于 Agent 的 `subagentAllowAgentIds` 生成 `agents.list[].subagents`。
...
`openclawAgentModels.buildAgentEntry` 负责把 Workhorse AI Agent 转成 OpenClaw Agent entry。
...
2. Workhorse AI final sync 会回填后续自动推进产生的 `sessions_spawn`、`sessions_yield` 工具结果，避免只记录第一段 spawn。
