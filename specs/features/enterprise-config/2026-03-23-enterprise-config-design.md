# Workhorse AI ToB Enterprise Config Support

## Overview

Enable Workhorse AI to support enterprise (ToB) deployment by reading a pre-configured package on startup, syncing model/IM/skill/agent/MCP configurations into SQLite, and locking down the UI so users can work immediately without manual setup.
...
**Platform paths**:
- macOS: `~/Library/Application Support/Workhorse AI/enterprise-config/`
- Windows: `%APPDATA%/Workhorse AI/enterprise-config/`
...
**Goal:** Enable Workhorse AI to read a pre-configured enterprise package on startup, sync model/IM/skill/agent/MCP configs into SQLite, and lock down the UI — so enterprise users can run the app with zero manual setup.
...
```bash
mkdir -p ~/Library/Application\ Support/Workhorse AI/enterprise-config/skills/test-skill
mkdir -p ~/Library/Application\ Support/Workhorse AI/enterprise-config/agents
mkdir -p ~/Library/Application\ Support/Workhorse AI/enterprise-config/mcp
```
...
```bash
rm -rf ~/Library/Application\ Support/Workhorse AI/enterprise-config
```
