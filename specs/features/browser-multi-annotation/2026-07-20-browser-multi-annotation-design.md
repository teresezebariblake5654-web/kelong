...
> 日期：2026-07-20
>
> 适用范围：Workhorse AI Cowork 内置浏览器与消息输入区
>
...
Workhorse AI 当前的浏览器标注流程一次只能选择一个页面元素。用户保存注释后，标注模式立即结束，系统把元素信息、用户评论等技术文本直接插入消息输入框，并把截图作为普通图片附件加入消息。
...
Workhorse AI 不要求逐字复制这些内部实现，但凡与参考行为不同的地方都必须在本文中标为明确的产品或运行时取舍。
...
- 批次和注释都使用稳定 UUID。
- 20 条是 Workhorse AI 的产品限制，不作为 ChatGPT 参考行为；10 条只表示截图 compact 阈值。
...
其中 20 条、2,000 字符、2 MB 等是 Workhorse AI 产品或工程限制；1,024/1,280/768、32/16 和 1,000 ms 来自当前参考版本，可在跨平台视觉回归后调整，但调整必须保留集中常量和测试。
...
- 不通过 `contextBridge` 向网页主世界暴露任何 Workhorse AI API。
...
- 网页脚本无法访问 Workhorse AI 主进程 IPC 或 Node API。
