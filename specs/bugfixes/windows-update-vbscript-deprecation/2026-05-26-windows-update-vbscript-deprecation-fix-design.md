...
用户反馈 Workhorse AI 的 Windows 更新脚本仍在使用 VBScript，而 Windows 11 正在逐步淘汰 VBScript 支持。截图中出现两个关键信息：
...
4. `.vbs` 再隐藏启动 `powershell.exe`，由 PowerShell 等待 Workhorse AI 退出后打开 NSIS 安装器。
...
因此用户说的“Workhorse AI 还在使用 vbs”是准确的。问题不在 NSIS 安装包本身，也不是手动下载链接，而是应用内“下载完成后点击立即更新”的 Windows 安装启动链路。
...
1. NSIS 安装器需要在 Workhorse AI 当前进程退出后再运行，避免安装时文件仍被占用。
2. 等待和启动安装器的辅助脚本不应显示额外控制台窗口。

为此现有实现使用了“两层脚本”：

```text
Workhorse AI main process
...
**When** 用户点击立即更新  
**Then** Workhorse AI 不应先退出  
**And** 更新状态进入安装失败，UI 可显示现有 `updateInstallFailed` 文案并允许用户重试或手动安装。

### 场景 4：当前 app 退出较慢

**Given** 当前 Workhorse AI 进程退出需要清理 OpenClaw Gateway、IM Gateway 或文件句柄  
**When** launcher 已启动  
...
- 保留 `detached: true` 和 `unref()`，使等待脚本不依赖 Workhorse AI 主进程生命周期。
...
1. 准备一个可检测到新版本的 Workhorse AI build。
2. 等待或手动触发更新下载。
3. 点击立即更新。
...
8. 确认 Workhorse AI 退出后 NSIS 安装器正常出现。
...
- [ ] Workhorse AI 当前进程退出后，NSIS 安装器正常出现。
