...
用户在 Cowork 会话中提问后，模型先输出一句类似“我先看看日志”的短回复，随后 Workhorse AI 界面认为本轮已经结束。用户需要再发送“继续”，模型才会继续输出完整分析。

从日志看，这不是模型真正停止，也不是单纯 UI 没有展示压缩进度。OpenClaw 后续仍然在同一个 run 中触发上下文溢出、自动压缩，并继续生成完整回答；但 Workhorse AI 主进程已经把该 run 标记为 completed，后续 assistant stream 被 closed-run guard 当作迟到事件丢弃。
...
4. `17:15:11`，OpenClaw 发出 `chat final`，`finalTextLen=109`。Workhorse AI 将 session 状态切到 `idle`，原因是 `run_completed`，随后执行 turn cleanup。
...
3. Workhorse AI 按普通完成路径调用 `deferChatFinalCompletion()`，短 grace 后将 session 标记 completed。
...
2. OpenClaw 自动压缩、retry 或 continuation 期间，Workhorse AI 应保持当前 turn 可恢复。
...
**Then** Workhorse AI 不应提前把 session 标记 completed  
...
**Then** 用户停止优先级最高，Workhorse AI 不应恢复该 turn  
