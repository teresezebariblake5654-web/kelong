...
同一个 IM 群聊中拉入多个机器人，并且这些机器人分别绑定到不同 Agent 时，OpenClaw 可以正确按 `sessionKey` 路由到目标 Agent，但 Workhorse AI 本地会话映射只按 `(platform, im_conversation_id)` 查找，导致不同 Agent 的同群会话被压成一条本地 mapping。
...
因此非 main Agent 的群聊消息虽然能在移动端收到回复，但不会同步到正确的 Workhorse AI 本地会话。
...
**Then** OpenClaw 的 `agent:<agentId>:feishu:group:<chatId>` 会话应同步到该 Agent 名下的 Workhorse AI 会话，而不是复用 main Agent 的同群会话。
...
1. 跳过 Workhorse AI 本地 session key。
...
3. 自定义 Agent 的群聊消息出现在该 Agent 名下的 Workhorse AI 会话。
