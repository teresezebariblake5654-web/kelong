...
后续验证还发现，已经保存在 OpenClaw cron state 中的旧任务不会重新经过创建/编辑归一化。用户点击“立即运行”，或旧任务自然到点执行，仍会使用旧 job 配置。结果可能是 IM 侧收到消息，但 Workhorse AI 侧没有展示在期望的群聊会话中。

进一步排查 OpenClaw 当前行为后确认：飞书 cron delivery 的发送目标如果是裸 `oc_...` 群聊 id，IM 发送可以成功，但 OpenClaw 侧 delivery mirror 会把这次出站消息记录到一个 `direct:<oc_...>` 形态的会话，而不是既有的 canonical `group:<oc_...>` 会话。OpenClaw Web UI 也会展示这个独立会话。因此 Workhorse AI 不应在普通会话列表层强行隐藏或合并该会话；更自然的短期策略是在定时任务候选通知渠道中隐藏这类 fake direct，避免用户配置任务时选错。
...
不带账号前缀。定时任务选择 Bot 实例时，不能通过给群聊 session key 增加 accountId 解决归属问题，只能结合 Workhorse AI 本地 mapping 和当前 Bot 绑定关系判断。

OpenClaw cron job 是持久化配置。创建/编辑任务会经过 Workhorse AI IPC handler，但手动运行只调用 `cron.run(id)`，自然到点执行则完全由 OpenClaw cron 调度。两条路径都不会重新执行创建/编辑时的 IM announce 归一化。
...
因此 Workhorse AI 不应在 `im_conversation_id` 或 OpenClaw canonical session key 层面发明群聊 account 前缀。群聊的 Bot 归属只能在 Workhorse AI 侧通过 mapping 元数据或当前实例绑定关系辅助判断。

### 2.1 飞书 delivery mirror 现状

当前 Workhorse AI pinned runtime 使用 OpenClaw v2026.6.1 与 `@larksuite/openclaw-lark@2026.6.10`。实测和源码排查结果：
...
因此当前 Workhorse AI 侧采用“配置候选隐藏 fake direct，普通会话列表保留 OpenClaw 实际会话”的策略。这样既避免定时任务配置误选，又不篡改 OpenClaw 真实会话结构。
...
如果 OpenClaw delivery mirror 把裸群聊 id 记录成 `direct:<chatId>` 形态，Workhorse AI 的 channel session sync 层不再全局合并到既有 accountless `group:<chatId>`/`channel:<peerId>` mapping。普通会话列表保留 OpenClaw 实际产生的会话；只有定时任务表单候选会隐藏同 peer fake direct。
...
1. 只处理 `delivery.mode = announce` 且能解析到 Workhorse AI IM 平台的任务。
...
OpenClaw delivery mirror 只追加 transcript，不一定更新 session store 中的 `updatedAt`。因此 Workhorse AI 不能只依赖最近活跃会话轮询，还要在收到 delivered cron finished event 后定向同步实际 mirror transcript：
...
OpenClaw 当前还会按最近 assistant 文本对 delivery mirror 去重：若不同执行输出完全相同，IM 仍可能重复收到，但 transcript 不追加新消息。本次不在 Workhorse AI 侧合成消息，也不修改该 OpenClaw 行为。
...
| Bot 实例未显式绑定 Agent | 按 OpenClaw/Workhorse AI 默认逻辑视为 `main` Agent |
...
5. 旧飞书群聊定时任务不重新编辑，点击“立即运行”后能成功发送到 IM；若 OpenClaw 仍写出 fake direct mirror，Workhorse AI 普通会话列表保持与 OpenClaw 一致。
...
9. 飞书群聊 cron 输出发生变化时，Workhorse AI 同步 OpenClaw 实际 fake direct transcript，不再错误刷新 canonical group 历史。
