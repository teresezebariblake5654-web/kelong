...
用户在 Cowork 任务执行过程中触发 OpenClaw 自动上下文压缩时，Workhorse AI 当前会在最终回答后方展示一条 `Compaction` 或上下文压缩完成提示。
...
3. **生命周期判断不完整**：OpenClaw 可能在一次 `lifecycle end` / 空 `chat.final` 之后才触发 context overflow auto-compaction，并使用同一个 `runId` 继续 retry。Workhorse AI 如果提前把该 `runId` 标记为 closed，后续 retry 的 assistant 文本会被当成 late event 丢弃，UI 停在中间态文本。

2026-05-20 复现日志中，OpenClaw 并没有卡死。实际时间线如下：

| 时间 | 事实 | 影响 |
|---|---|---|
| 12:08:27 | 同一 `runId` 第一次 `lifecycle=end`，`chat.final` 文本为空 | Workhorse AI 进入可完成路径 |
...
| 12:09:14 | Workhorse AI 从 history 同步到一句中间态文本并触发 complete | UI 显示“分析大致完成了...”并结束 turn |
| 12:09:22 | OpenClaw 第二次 compaction 后再次用同一个 `runId` retry | 后续事件仍属于同一用户任务 |
| 12:09:31-12:09:40 | OpenClaw 输出真正最终回答并成功结束 | Workhorse AI 因 `recentlyClosedRunIds` 丢弃这些文本 |

因此，本修复不能只解决“Compaction 卡片展示位置”。必须同时修正 `chat.final`、`lifecycle=end`、auto-compaction retry 和 `recentlyClosedRunIds` 之间的状态机关系。

2026-05-20 12:50 再次复现证明还存在另一条未覆盖路径：第一次 attempt 的 `chat.final` 已能被正确延迟，但第二次 attempt 没有发送 `chat.final`，只发送了 `lifecycle=end`。Workhorse AI 的 lifecycle fallback 在约 800ms 后从 history 同步到一句短中间态 assistant 文本，随后直接 complete。约 11 秒后 OpenClaw 完成第二次 auto-compaction 并用同一个 `runId` retry 时，Workhorse AI 已经把该 run 作为不可恢复 closed run 处理，导致新的 `lifecycle=start` 被丢弃。

| 时间 | 事实 | 影响 |
|---|---|---|
| 12:48:57 | 第一次 attempt 收到短 `chat.final`，tool result 约 210K 字符 | Workhorse AI 正确延迟 final，等待 retry |
...
| 12:49:53 | lifecycle fallback 从 history 同步到短中间态文本并 complete | Workhorse AI 提前关闭 turn |
...
- history 同步会把 OpenClaw 内部 UI 分隔符泄漏到 Workhorse AI 聊天流。
- renderer 只能根据 `metadata.kind` 区分压缩消息；裸 `Compaction` 无法走专门样式。

第二层是生命周期模型问题：**Workhorse AI 把 `runId` 当成单次完成边界，但 OpenClaw auto-compaction retry 会复用同一个 `runId`**。
...
- 不修改模型上下文窗口计算。
- 不要求第一版修改 OpenClaw/gateway 协议；如果协议没有结构化 compaction/retry 事件，Workhorse AI 需要有保守兜底。
...
**When** Workhorse AI 收到 `stream=compaction phase=start`  
...
**When** Workhorse AI 收到 `stream=compaction phase=end completed=true`  
...
**When** Workhorse AI 执行 `syncSystemMessagesFromHistory()`  
...
**When** Workhorse AI 收到同一 `runId`、同一 `sessionKey` 的新 `lifecycle phase=start`  
...
**Then** Workhorse AI 仍应通过 closed-run / turn-token / session status 防重机制丢弃它们  
**And** 不应重新打开已完成会话。
...
**When** Workhorse AI 在 `lifecycle=end` fallback 中从 `chat.history` 同步到短 assistant 文本  
**Then** 该文本只能作为中间态候选展示  
**And** Workhorse AI 必须进入 recoverable retry wait，而不是直接 complete  
...
| OpenClaw event | Workhorse AI 行为 |
|---|---|
...
2. 如果收到 recently closed run 的 `phase=start`，且满足以下条件，可以解除 closed 状态并恢复接收：
   - `sessionKey` 能解析到同一个 Workhorse AI session。
...
这条规则不能只依赖 `turn.hasContextMaintenanceTool`。OpenClaw 内部 auto-compaction 可能不会表现为 Workhorse AI 的显式 maintenance tool。
...
这部分是本 bug 的核心。只依靠 compaction stream 的方案不够，因为 2026-05-20 日志中的失败路径没有给 Workhorse AI 足够明确的结构化 compaction event。
...
- `stream=compaction phase=start/end`：确保 Workhorse AI 不必从 stdout 或 usage count 推断。
- `willRetry=true`：在 `lifecycle=end` 或 `chat.final` 上明确告诉客户端这不是 user turn 结束。
- `retryReason=context_overflow`：让 UI 和日志能解释等待原因。

有了这些字段后，Workhorse AI 可以从启发式 retry reopen 迁移到协议驱动状态机。
