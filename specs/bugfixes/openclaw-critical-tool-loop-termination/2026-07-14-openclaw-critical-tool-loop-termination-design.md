...
Workhorse AI 当前固定使用 OpenClaw `v2026.6.1`。该版本已经具备原生工具循环检测：当重复调用达到 critical 阈值时，`before_tool_call` 返回 `deniedReason: "tool-loop"`，并记录类似 `Session execution blocked` 的日志。
...
1. 新 patch 能与 Workhorse AI 其余 `v2026.6.1` patches 一起从干净 tag 顺序应用。
...
7. Workhorse AI patch 内容测试和强校验通过。
