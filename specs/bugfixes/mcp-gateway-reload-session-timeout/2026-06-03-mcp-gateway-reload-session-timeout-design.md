...
它们的共同根因是：MCP 开启或切换后，OpenClaw gateway 正处于 MCP 配置 reload/restart 窗口，Workhorse AI 仍然允许用户立即发起对话，导致握手或关键 session RPC 卡住。

### 1.2 根因

Workhorse AI 与 OpenClaw 对 MCP 配置变更的处理语义不一致。

当前 Workhorse AI 侧 MCP handler 在 create/update/delete/toggle/retry 时调用：
...
因此 MCP 配置变化会被 Workhorse AI 判断为：
...
1. Workhorse AI 把 MCP 配置写入 `openclaw.json` 后认为可以热加载。
2. OpenClaw runtime 认为 `mcp` 变更需要重启，并进入 reload/restart pending 状态。
3. 用户立即发起 Cowork 对话。
4. Workhorse AI 开始 `ensureGatewayClientReady()`、`sessions.patch`、`chat.send` 前置流程。
...
**Given** Workhorse AI 正在等待 gateway client `hello-ok`  
...
### FR-3：MCP 变更应由 Workhorse AI 主动执行 hard restart

对于 MCP create/update/delete/toggle/launch-ready/retry 等导致的 `mcp` 配置变化，Workhorse AI 应主动进入已有 hard restart 流程，而不是依赖 OpenClaw 自己在 runtime 内部检测 reload。
...
4. 如果有 active workload，进入 Workhorse AI 自己的 deferred restart 队列。
5. restart 完成后重新建立 gateway client。

这样可以避免两套状态机同时存在：

- Workhorse AI 认为 hot reload 完成
...
当 close 原因来自 Workhorse AI 主动 hard restart 或 MCP config apply barrier 时，应立即取消旧 `gatewayReadyPromise`，并在新 gateway ready 后创建新的 client。
...
| Workhorse AI 主动 restart | reject 旧 ready promise，并等待新 gateway |
...
| OpenClaw reload 仍自行检测到 mcp restart | Workhorse AI 日志应能显示是否漏判 MCP restart impact |
...
- 启用、停用、重试 MCP 后，日志中不再出现 Workhorse AI 判断 `NO RESTART, hot-reload only` 但 OpenClaw 随后输出 `config change requires gateway restart (mcp)` 的矛盾状态。
