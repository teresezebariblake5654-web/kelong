...
用户在 Workhorse AI Cowork 中执行 OpenClaw 任务后，tool result 回填出现两类相反表现：
...
- `18:14:45.988` Workhorse AI 记录 `agent lifecycle end fallback completed a turn that missed chat final`
- `18:14:46.021` Workhorse AI 发出 session complete

因此问题不是 run 仍在执行，而是 Workhorse AI 本地消息中缺少对应的 `tool_result`，导致 renderer 把未配对的 `tool_use` 继续渲染为运行中。

第二个现场中，当前 turn 实际只执行了一个 `memory_search`，但 `chat.final` 后 Workhorse AI 又从 `chat.history` 写入多个旧 `exec/read` 的 `tool_result`。这些结果没有本地配对的 `tool_use`，因此 renderer 走孤立结果分支，展示成多个 `执行结果` 块。
...
| 字段 | 值 |
|---|---|
| Workhorse AI sessionId | `269ed742-76f0-457a-bb54-c0ebd7fedd11` |
...
3. 对这两个 tool call，gateway/main 日志中没有对应的 `stream=tool result:read` 事件，也没有 Workhorse AI 的 `tool_result` 转发记录。
...
第一个现场正好走了第二条路径。OpenClaw 的最终 `chat.history` 是完整的，但 Workhorse AI fallback 同步没有消费里面的 `toolResult`，导致本地消息缺口保留下来。

第二个现场走了第一条路径。当前 turn 的 `memory_search` 已经完整结束，但 `chat.history` 窗口里还包含更早 turn 的旧 `toolResult`。由于回填逻辑没有当前 turn 归属校验，这些旧结果被写入当前 Workhorse AI 消息流，形成孤立 `执行结果`。
...
1. 当 OpenClaw 漏发或延迟发送实时 `stream=tool result`，但最终 `chat.history` 中已有 `toolResult` 时，Workhorse AI 应补齐本地 `tool_result` 消息。
2. 当 `chat.history` 包含旧 turn 的 `toolResult` 时，Workhorse AI 不应把未知 `toolCallId` 写入当前本地消息流。
...
**And** Workhorse AI 已创建本地 `tool_use` 消息  
...
**Then** Workhorse AI 应创建本地 `tool_result` 消息  
...
**Then** Workhorse AI 应更新已有 `tool_result` 内容  
