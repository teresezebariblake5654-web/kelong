...
- Agent 创建后，Workhorse AI 的 config sync 会把 main `USER.md` 再复制到新 Agent workspace；
- 由于 OpenClaw 使用 `writeFileIfMissing` 创建 bootstrap 文件，Workhorse AI 先写入后，
  OpenClaw 后续不会再写自己的默认 `USER.md` 模板。

### 1.2 根因

根因不是 SQLite 中 Agent 数据串写，也不是 OpenClaw runtime 只支持共享 `USER.md`。根因是
Workhorse AI 集成层把 `USER.md` 的所有权和作用域接错了。

当前代码路径中：

| 文件 | 当前 Workhorse AI 行为 | 正确语义 |
|------|---------------------|----------|
...
5. 新建 Agent 未显式填写“关于你”时，Workhorse AI 不主动创建 `USER.md`，由 OpenClaw 在
...
**Then** Workhorse AI 不写 main `USER.md`

**And** Workhorse AI 不写新 Agent 的 `USER.md`
...
**Then** Workhorse AI 将该内容写入新 Agent 的 `workspace-{agentId}/USER.md`
...
| OpenClaw 默认模板更新 | Workhorse AI 不内置模板，因此自动跟随 OpenClaw runtime |
