...
从日志看，这次失败不是模型没有生成回复，而是 Workhorse AI 在真正调用 `chat.send` 之前，先执行 OpenClaw `sessions.patch` 更新会话模型。该 RPC 在客户端默认 30 秒内没有返回，导致 `runTurn()` 直接进入 error，用户消息没有被发送给模型。
...
5. Workhorse AI 抛出 `gateway request timeout for sessions.patch`。
...
/Users/wangning/Library/Application Support/Workhorse AI/openclaw/state/agents/main/sessions/sessions.json.lock
```
...
Workhorse AI 需要维护轻量 gateway RPC health state：
...
Workhorse AI 可以先做降级和限流，但根因仍在 OpenClaw session 写锁持有过久。
