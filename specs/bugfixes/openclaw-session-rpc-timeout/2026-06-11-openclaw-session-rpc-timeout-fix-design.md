...
结论：该机器存在进程级资源饥饿（内存换页/磁盘饱和/杀软实时扫描叠加），属环境问题，Workhorse AI 无法在代码层根治；但发送链路应当对"引擎慢"具备容错——**gateway 是自愈的、patch 是幂等的且最终会成功，客户端却在 30 秒就放弃并丢弃消息**，这是本次要修复的缺陷。
...
```text
OpenClaw 引擎响应缓慢，消息尚未发送。请等待 1~2 分钟后重新发送；若频繁出现，请检查系统内存与磁盘占用，并将 Workhorse AI 加入杀毒软件白名单。
```

英文：

```text
The OpenClaw engine is responding slowly and your message has not been sent. Please wait a minute or two and resend. If this happens frequently, check system memory and disk usage, and add Workhorse AI to your antivirus allowlist.
```
