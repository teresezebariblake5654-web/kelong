...
用户在 Workhorse AI 桌面端发起的会话（managed session）中，流式显示的文本会丢失重复字符。典型症状：
...
Workhorse AI 的 `mergeStreamingText` 曾有相同的 overlap 检测（commit d218d56b 已移除）。但由于接收的 `chat.delta` 数据本身已经被网关损坏，即使 Workhorse AI 侧完美拼接也无法修复丢失的字符。
