...
4. DeepSeek 与 Xiaomi 没有 provider-owned replay policy 配置，也没有在 Workhorse AI 侧显式声明“这类模型需要 reasoning 回放”。

### 2.2 已有能力

Workhorse AI 已有几段基础能力可复用：
...
3. 打包 runtime 的插件列表包含 `deepseek`，但没有 `xiaomi`。Workhorse AI 的 `scripts/prune-openclaw-runtime.cjs` bundled extension 保留名单也漏掉了 `xiaomi`，导致打包产物会剪掉 Xiaomi provider extension。
...
`scripts/prune-openclaw-runtime.cjs` 的 bundled extension 保留名单没有包含 `xiaomi`，导致发布包中 Xiaomi provider extension 被删除。即便源码里的 `extensions/xiaomi` 已经补齐 provider hook，打包产物也不会加载这些 hook。这个问题必须在 Workhorse AI 仓库内用 prune 测试锁住。
...
### 5.1 Workhorse AI 侧 provider 配置与打包保留
...
### 5.2 Workhorse AI 侧 provider reasoning 元数据
...
如果后续升级 OpenClaw 到包含官方修复的版本，应优先删除本地 patch，并用 Workhorse AI 侧测试锁定行为。
