...
当 AI Agent 执行产出大量输出的工具调用（如 `node -e` 生成 2MB+ JSON）时，OpenClaw gateway 的 `tick` 心跳事件被 event loop I/O 饿死，Workhorse AI 客户端的 `TickWatchdog` 误判连接已死并主动断连，UI 显示"AI 引擎连接中断"。
...
| Workhorse AI 客户端 | `handleGatewayEvent` 仅在 `event === 'tick'` 时更新活跃时间戳，忽略其他事件 | `openclawRuntimeAdapter.ts:3806` |
...
| 根因在上游 | gateway event-loop starvation 需 openclaw 侧修复（#83366 OPEN），Workhorse AI 侧无法从根本解决 |
