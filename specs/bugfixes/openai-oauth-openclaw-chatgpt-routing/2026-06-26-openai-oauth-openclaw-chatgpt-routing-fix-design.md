...
Workhorse AI 升级到新版 OpenClaw 后，原先已经支持的 OpenAI ChatGPT OAuth 登录模式不再兼容。用户完成 ChatGPT OAuth 登录后，Workhorse AI 仍会向 OpenClaw 写出旧版 Codex provider 配置：
...
| 项 | 旧 Workhorse AI 配置 | 新版 OpenClaw 期望 |
|----|------------------|-------------------|
...
OAuth token 文件本身不是根因。Workhorse AI 仍会把 `CODEX_HOME` 指向 app-managed `<userData>/codex`，新版 OpenClaw 也仍能从该目录读取 `auth.json` 中的 `tokens.access_token` / `tokens.refresh_token`，并识别为 `provider: "openai"` 的 OAuth credential。

### 1.3 目标

1. OpenAI ChatGPT OAuth 登录后，OpenClaw gateway 可以正常启动。
2. Workhorse AI 生成新版 OpenClaw 接受的 canonical model route：`openai/gpt-*`。
...
**When** Workhorse AI 启动新版 OpenClaw gateway  
...
**Then** Workhorse AI 同步 OpenClaw 配置时不写 API key，不写旧 `openai-codex` provider，而是让 OpenClaw 通过 `CODEX_HOME` 读取 `openai` OAuth profile

### 场景 3: 旧会话中保存了 `openai-codex/gpt-*`

**Given** OpenClaw sessions store 或 Workhorse AI agent 配置中存在旧模型 ref  
**When** 配置同步或会话恢复发生  
**Then** Workhorse AI 将受管 session route 迁移为 `provider=openai`、`model=gpt-*`，或者在读取旧 ref 时兼容解析到当前 OpenAI OAuth 模型
...
Workhorse AI 不应继续向 `openclaw.json` 写出：
...
```text
CODEX_HOME=<Workhorse AI userData>/codex
```

### FR-4: 迁移旧 model ref

Workhorse AI 需要兼容或迁移以下旧形态：
...
迁移范围优先覆盖 Workhorse AI 受管 session store 和 agent 配置。非受管 OpenClaw 内部文件可以依赖新版 OpenClaw `doctor --fix` 作为额外修复工具，但不能把 doctor 作为 Workhorse AI 新配置输出的前置条件。
...
### 4.3 移除 Workhorse AI 手写 Codex headers 依赖
...
因此 Workhorse AI 侧的 `buildOpenAICodexHeaders()` 不应再作为新版路径的必要条件。推荐策略：
...
`OpenClawConfigSync.syncManagedSessionStore()` 已会基于 `providerSelection.primaryModel` 同步 Workhorse AI 受管 session route。修复后 `providerSelection.primaryModel` 会从 `openai-codex/gpt-*` 变为 `openai/gpt-*`，因此该路径可以覆盖多数受管 session。
...
4. 对 channel session 或非 `agent:*:lobsterai:*` session，只做安全兼容解析，不做过度重写，避免误改 OpenClaw 非 Workhorse AI 管理的状态。

### 4.6 不把 `openclaw doctor --fix` 作为主修复

新版 OpenClaw 文档说明 `doctor --fix` 可以迁移 legacy `openai-codex/*` model refs 和 `openai-codex:*` auth profiles。但 Workhorse AI 的 `openclaw.json` 是由 `OpenClawConfigSync` 持续生成的：

```text
Workhorse AI config sync -> openclaw.json
```

如果只运行 doctor，下一次 Workhorse AI sync 仍会写回旧配置。因此 doctor 只能作为用户已有 OpenClaw 状态的补充修复，不是产品内兼容的根治方案。
...
| 旧 `openai-codex` provider 残留在 existing `openclaw.json` | 下一次 Workhorse AI sync 应输出新 `openai` provider 并覆盖 managed config |
...
2. 不要求用户手动运行 `openclaw doctor --fix` 才能启动 Workhorse AI。
