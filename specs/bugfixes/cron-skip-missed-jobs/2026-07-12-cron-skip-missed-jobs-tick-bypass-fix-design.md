...
用户在设置中开启「跳过未执行任务」(`skipMissedJobs`) 后，每次启动 Workhorse AI，仍然会出现：
...
`skipMissedJobs` 是 Workhorse AI 通过版本化 patch 给 OpenClaw 增加的能力，完整链路：
...
skip 分支执行时以 info 级日志输出：跳过的 job 数量、每个 job 的 `id`/`name`、原 `nextRunAtMs`、推进后的 `nextRunAtMs`。cron event 流如可行则同步发出等价事件，便于 Workhorse AI 端未来在任务运行历史中展示「已跳过 N 次」。
...
- Workhorse AI 侧 `cronSkipMissedJobs.test.ts` 的文本断言同步更新，至少覆盖「skip 分支包含快进调用」的关键代码行，防止未来 rebase patch 时把快进逻辑丢掉。
...
- **Workhorse AI 侧启动后经 cron API 批量禁用/启用或改写任务**：与 gateway 首次 tick 存在竞态（本次实测 tick 在 listening 后 ~5 秒就收割了任务），且把 OpenClaw 内部调度语义泄漏到产品层。
...
6. 端到端复现用户原始场景：白天关闭应用错过多个任务，晚间启动 Workhorse AI，浏览器不弹出、无 token 消耗、无 stale-delivery 丢弃日志。
7. OpenClaw 侧新增行为测试通过；Workhorse AI `npm test` 通过；`npm run compile:electron` 通过（若触及主进程文件）。
...
- 正例指纹：`cron: skipping missed jobs after restart` + 每任务 `old nextRunAtMs → new nextRunAtMs`。
  ...
  **不进入 gateway stdout / Workhorse AI main log**（上游自己的 catch-up 日志同样如此，属 logger 管道
