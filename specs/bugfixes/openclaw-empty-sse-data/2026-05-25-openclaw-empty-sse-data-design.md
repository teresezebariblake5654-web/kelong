...
用户在 Workhorse AI Cowork 中执行一个较长任务时，工具调用本身已经成功，但界面随后展示系统错误：
...
| `main-2026-05-21.log` | Workhorse AI 主进程透传和 session 状态日志 |
...
7. gateway 发出 `stream=lifecycle phase=error error=Unexpected end of JSON input`，Workhorse AI UI 显示该错误。
...
因此，问题根因位于 **自定义 provider / AIGW / proxy 返回了不规范空 data 帧，以及 OpenClaw parser 没有容忍该空帧** 这一层。Workhorse AI 主进程只是通过 lifecycle error fallback 把 OpenClaw 的错误透传到了 UI。
...
6. Workhorse AI UI 不再直接暴露低层 `Unexpected end of JSON input`，至少应在兜底路径中显示更可诊断的模型流格式错误。
...
**And** Workhorse AI 不应显示 `Unexpected end of JSON input`
...
**And** 对 Workhorse AI 返回明确错误，例如“模型流式响应一直为空”  
...
### FR-6: Workhorse AI 侧兜底错误提示应更可诊断

如果 OpenClaw 仍然向 Workhorse AI 抛出底层 parse 错误，`OpenClawRuntimeAdapter` 可以在不吞错的前提下把用户可见文案包装得更明确。
...
因此重点路径是自定义 provider 的 OpenAI-compatible streaming 适配层，而不是 Workhorse AI 的 `OpenClawRuntimeAdapter`、renderer 或工具展示层。

如果该 parser 位于 OpenClaw 独立仓库或 bundled runtime 中，应在 OpenClaw 源码修复后更新 Workhorse AI `package.json` 中 pinned OpenClaw 版本，并通过 Workhorse AI 集成测试验证。
...
### 4.5 Workhorse AI 兜底映射

若短期内无法升级 OpenClaw runtime，可在 Workhorse AI 侧做低风险兜底：
...
- `src/main/libs/agentEngine/openclawRuntimeAdapter.test.ts`：如果增加 Workhorse AI 侧错误映射，需要补测试
...
- Workhorse AI session 不应永久保持 running。
...
3. 在 Workhorse AI 中使用自定义 OpenAI-compatible provider 做一次手动验证：
   - 长任务工具调用后继续生成。
   - 插入空 data 心跳。
