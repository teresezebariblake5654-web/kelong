...
用户反馈“在没有定时任务、没有主动发起任务的时段，Workhorse AI 仍然持续消耗 token”。本次网关侧抓到的请求证明，异常消耗不是普通心跳或 `cron.list` 轮询，而是一次真实模型请求在持续重放异常上下文。
...
1. 旧版本 OpenClaw/Workhorse AI 没有对连续 `Aborted` 工具结果设置硬断路。
   - 工具执行已经无法取得结果。
   - 模型仍被允许反复选择同类工具。
...
2. 当前 OpenClaw 虽已有 tool loop detection / circuit breaker 能力，但默认关闭。
   ...
   - Workhorse AI managed config 当前没有强制开启该能力。
...
- `src/main/libs/openclawHistory.ts` 已过滤 heartbeat prompt/ack、`NO_REPLY` 和空文本 assistant，但这主要覆盖 Workhorse AI 对 `chat.history` 的 UI 同步，不等价于 OpenClaw 发送模型请求前的上下文清洗。
...
2. Workhorse AI managed runtime 默认开启 OpenClaw tool loop detection / circuit breaker。
...
**Given** 用户点击停止当前任务  
**When** Workhorse AI 调用 `chat.abort` 或 OpenClaw 工具收到 abort signal  
**Then** 当前 run 应进入终止态  
...
**When** Workhorse AI 只进行 gateway 状态轮询、session list 或 cron list  
**Then** 不应产生新的模型请求  
...
Workhorse AI 生成的 managed OpenClaw 配置必须启用工具循环检测。
...
2. 向 gateway / Workhorse AI 发出 final error 或 lifecycle error。
3. 清理 active turn。
...
当 Workhorse AI 调用 `stopSession()` 或 gateway 收到 `chat.abort` 后：
...
- Workhorse AI active turn cleanup 不应留下可继续执行的 pending retry。

### FR-7: Workhorse AI history sync 不引入异常消息

Workhorse AI 从 `chat.history` 同步 UI 消息时，应继续忽略：
...
### 4.1 Workhorse AI managed config 启用 loop detection
...
如果 OpenClaw 后续把 loop detection 默认值改为 true，Workhorse AI 仍应显式写入该配置，避免旧 runtime 或用户 overlay 下行为漂移。
...
该函数应在发送模型请求前运行，而不仅在 `chat.history` 返回给 Workhorse AI 时运行。

### 4.5 Workhorse AI 收到断路错误后的 UI 收口

当 OpenClaw 因断路器终止 run 时，Workhorse AI `openclawRuntimeAdapter.ts` 应按错误完成路径处理：
...
1. Workhorse AI 生成的 managed OpenClaw config 中包含 `tools.loopDetection.enabled=true`。
...
在本地运行 OpenClaw gateway + Workhorse AI：
