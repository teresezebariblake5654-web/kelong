...
`~/Library/Application Support/Workhorse AI/updates/` 下的 DMG,系统弹出的真实错误
即根因;重启 Mac 可清除附加残留。
...
- 不再假设挂载点行的位置、分隔符和 `/Volumes` 前缀,天然兼容 HFS+(2 行)与
  APFS(4 entity)两种 DMG 结构、卷名含空格/中文/重名(`Workhorse AI 1`)等情况;
...
```json
{
  "system-entities": [
    ...
    { "dev-entry": "/dev/disk5s1", "mount-point": "/Volumes/Workhorse AI" }
  ]
}
```
...
| 卷名含空格/中文/重名(`/Volumes/Workhorse AI 1`) | plist 中是完整字符串,不受影响 |
