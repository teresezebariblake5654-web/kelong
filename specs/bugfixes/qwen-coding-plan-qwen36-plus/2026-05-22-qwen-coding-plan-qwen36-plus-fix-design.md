...
用户在 Workhorse AI 设置里启用 Qwen Coding Plan 后，同一套 API key 和 Coding Plan endpoint 下：
...
因此，当前失败不是因为 Workhorse AI 没有把 `qwen3.6-plus` 写入配置；即使写入了，也会被 OpenClaw runtime 过滤或 suppress。

### 1.3 核心判断

既然 `qwen3.5-plus` 在同一个 Coding Plan endpoint 下可用，最小修复不应优先改 provider id，也不应在 Workhorse AI UI 里隐藏 `qwen3.6-plus`。
...
### FR-4：不改 Workhorse AI 的 provider id 映射

本次修复不要求把 Workhorse AI 的 `qwen-portal/qwen3.6-plus` 改成 `bailian-coding-plan/qwen3.6-plus`。
...
### FR-5：Workhorse AI UI 不应主动隐藏 qwen3.6-plus
...
在 Workhorse AI 的 runtime patch 目录新增：
...
### 4.5 Workhorse AI 主工程不做 provider id 重写
...
### 6.2 Workhorse AI runtime patch 验证
...
1. 在 Workhorse AI 设置页启用 Qwen Coding Plan。
...
2. 不把 Workhorse AI 的 Qwen provider 从 `qwen-portal` 改名。
