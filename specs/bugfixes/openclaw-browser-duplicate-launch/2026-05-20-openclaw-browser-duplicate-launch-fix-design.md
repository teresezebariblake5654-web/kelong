...
用户在 Cowork 任务中调用 browser 工具时，Workhorse AI 已经启动了一个独立的托管 Chrome profile：`openclaw`。这个托管浏览器启动后会先显示一个空白 tab，随后 browser 工具通过 CDP 打开真实目标页面。
...
**Then** Workhorse AI 可以启动一个 `openclaw` managed Chrome，并通过 CDP 打开目标页面。
...
- Workhorse AI 补丁：`scripts/patches/v2026.4.14/openclaw-browser-duplicate-launch.patch`
...
由于 Workhorse AI 通过 `scripts/apply-openclaw-patches.cjs` 对 pinned OpenClaw 版本应用补丁，修复不应只修改 `vendor/openclaw-runtime/*/dist`。
...
3. 将 OpenClaw browser 侧修复向上游同步，减少 Workhorse AI 长期维护补丁的成本。
