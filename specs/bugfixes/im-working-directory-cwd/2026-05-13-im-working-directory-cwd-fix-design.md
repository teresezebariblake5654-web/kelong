...
用户在 Workhorse AI 中给微信、飞书等 IM 渠道完成绑定后，从 IM 中询问“当前的工作目录是哪里”，回复显示的是默认的 Workhorse AI 目录，而不是用户在 Workhorse AI 中选择的任务工作目录。

同一用户在 Workhorse AI 桌面内新建任务并询问相同问题时，回复是正确的用户选择目录。

后续复查发现一个更具体的回归：IM 端现在不再总是回答 Workhorse AI 默认目录，而是统一回答主 Agent 的工作目录。这个结果仍然不正确，因为微信、飞书、钉钉、网易 IM 等不同 IM 入口可以绑定到不同 Agent。IM 入站任务的 cwd 必须由“该 IM 账号/实例/会话路由到的 Agent”决定，不能统一使用 main Agent。
...
所以桌面任务能正确回答 cwd，是因为每次由 Workhorse AI 主动调用 `chat.send` 时都会显式传入 session cwd。
...
3. Workhorse AI 通过 `OpenClawChannelSessionSync` 发现 gateway 里的 channel session，并在本地创建或复用 Cowork session。
...
- `OpenClawRuntimeAdapter.runTurn()` 会从 `session.cwd` 计算 `runCwd`，并把它传给 `chat.send`。这只覆盖 Workhorse AI 桌面主动发起的任务。
- IM channel plugin 入站路径不经过 Workhorse AI 的 `runtime.startSession()`，也不会使用 `OpenClawRuntimeAdapter` 的 `chat.send.cwd`。
...
- 当前 `OpenClawConfigSync` 写出的 `agents.defaults.workspace` 是 Workhorse AI 管理的 OpenClaw workspace，用于 Agent 记忆和 bootstrap 文件；用户选择的任务目录被写到 `agents.defaults.cwd` / `agents.list[].cwd`。
- 当前随包 OpenClaw v2026.4.14 的 plugin-sdk 类型已声明 `agents.defaults.cwd` / `agents.list[].cwd` 可作为 Agent run cwd，并且 reply run 参数也包含 `cwd`；因此 Workhorse AI 侧必须确保配置写出和 runtime 刷新正确。
...
这次“所有 IM 都回答 main Agent 工作目录”的现象，说明前一轮只把 `agents.defaults.cwd` 改为 main Agent 工作目录并不够。它让 fallback cwd 从 Workhorse AI 默认目录变成了 main Agent 目录，但没有保证非 main Agent 的 channel run 消费自己的 `agents.list[].cwd`。
...
**Then** 回复和工具执行 cwd 都应指向 `/repo/user-project`，而不是 Workhorse AI 默认目录或 OpenClaw workspace。
...
**When** 用户在 Workhorse AI 中把绑定 Agent 的工作目录改为 `/repo/new`  
**Then** 后续 IM 消息不应继续沿用 `/repo/old` 或默认目录。

### 场景 D: 桌面任务行为不回退

**Given** 用户在 Workhorse AI 桌面内新建任务  
...
**Then** 生成的最终文件应默认保存到 `/repo/agent-b` 或其子目录，而不是 Workhorse AI 应用目录、OpenClaw workspace、`~/.openclaw/workspace-*` 或临时目录。
...
不能只更新 Workhorse AI 本地 `cowork_sessions.cwd`，因为它不决定 IM 的真实执行目录。
...
Workhorse AI 之前已经把 OpenClaw workspace 与用户项目 cwd 解耦。修复不应简单地把 `agents.list[].workspace` 改成用户选择目录，否则会把 `AGENTS.md`、`IDENTITY.md`、`USER.md`、Agent 记忆和 inbound media 等 OpenClaw 工作区文件写入用户项目。
...
需要在 OpenClaw channel runtime 或 Workhorse AI 可维护的 channel glue 层新增统一解析函数，语义如下：
...
如果短期内无法修改钉钉 connector，则 Workhorse AI 写出 binding 时需要对这类自定义 matcher 平台生成它能识别的 binding 形态。但长期应收敛到 core routing，减少平台之间的路由差异。

### 4.5 Workhorse AI 写出正确 cwd 配置

`src/main/libs/openclawConfigSync.ts` 和 `src/main/libs/openclawAgentModels.ts` 需要保持以下输出：

- `agents.defaults.workspace`：Workhorse AI 管理的 main Agent workspace，例如 `{STATE_DIR}/workspace-main`。
...
- 如果 channel run 每轮都会从 Agent 配置重新解析 cwd，则只需要清理 Workhorse AI 的 `OpenClawChannelSessionSync` cache，并在下一轮同步时校正本地 `cowork_sessions.cwd`。
...
- Workhorse AI 同步到本地 Cowork session 后，文件链接解析应继续使用 `cowork_sessions.cwd`，且该值应与本轮 `resolvedCwd` 一致。
...
- Workhorse AI `CoworkSessionDetail` 对相对文件链接的 cwd 解析。
...
| 用户项目目录不存在 | Workhorse AI 侧保存时或运行前应提示；不能让 OpenClaw静默 fallback 到默认目录 |
...
8. Workhorse AI 桌面新建任务仍使用 `cowork_sessions.cwd` 快照，并通过 `chat.send.cwd` 生效。
9. OpenClaw workspace 文件仍写入 Workhorse AI 管理目录，不污染用户项目目录。
...
1. 在 Workhorse AI 中选择一个非默认目录，例如 `/tmp/lobsterai-cwd-check-a`。
2. 绑定微信到当前 Agent。
...
本次实现采用“Workhorse AI 配置写出 + OpenClaw runtime patch + channel plugin post-install patch”的组合：

1. Workhorse AI 继续写出 `agents.defaults.cwd` 和 `agents.list[].cwd`，并在 main Agent 工作目录变化时触发 gateway restart/refresh。
