...
用户在 Workhorse AI Cowork 中一次性提交多张图片附件时，界面提示：
...
当前 Workhorse AI 只校验了单张图片原始大小，不校验一次 `chat.send` 的总 payload 大小。多张图片各自没有超过单图限制，但合并为 base64 JSON payload 后超过 OpenClaw 网关 WebSocket 上限，最终触发 `1009`。
...
| 文件　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　| 作用　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　　|
| -------------------------------------------------------------------| -----------------------------------------------------------------------------------------------|
| `1780471049120lobsterai-logs-20260603-151636/main-2026-06-03.log` | Workhorse AI 主进程日志，包含 OpenClaw stdout/stderr 透传、`chat.send` 附件诊断和网关 close code |

关键失败链路 1：

1. `15:09:44` 用户新建 Cowork session，prompt 长度 15，图片附件 8 张。
2. Workhorse AI 主进程收到 8 张图片，base64 长度分别为：
...
6. Workhorse AI GatewayClient 收到：
...
**Then** Workhorse AI 应把错误分类为“本次消息过大”  
...
**Then** Workhorse AI 应继续使用现有网关断连、服务重启或网络错误提示  
**And** 不应误提示“本次消息过大”
...
OpenClaw 网关日志表现出的上限约为 30 MB 级别。Workhorse AI 应定义一个集中常量，例如：
...
5. 添加多张合计超过约 30 MB payload 的图片，确认发送前被 Workhorse AI 阻止。
6. 确认 UI 显示：
...
1. 多图合并 payload 超过安全上限时，Workhorse AI 不再把请求发给 OpenClaw 网关。
