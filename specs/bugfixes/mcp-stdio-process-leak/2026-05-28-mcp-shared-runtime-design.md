...
Workhorse AI 桌面端运行期间，Node.js 进程不断增殖。每新建一个对话（无论是否用到 MCP 工具），都会额外产生 N 个 Node.js 子进程（N = 已配置的 stdio MCP server 数量）。这些进程永远不会被释放，直到用户退出应用。
...
Workhorse AI 桌面端每个对话对应一个**独立的 gateway session key**（`agent:{agentId}:lobsterai:{uuid}`），新建对话 = 新 sessionId = 新 MCP 进程。
...
| 用户手动删除对话 | 否 | `onSessionDeleted` 只清理 Workhorse AI 侧状态，不通知 gateway |

结论：对于 Workhorse AI 桌面端的 gateway 模式，**不存在任何有效的 MCP 进程释放路径**。
