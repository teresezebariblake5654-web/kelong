...
用户反馈：Windows 覆盖安装 Workhorse AI 后，打开“设置 → IM 机器人 → 微信”会显示连接失败，错误为 `not configured`，必须重新扫码后微信连接才恢复正常。
...
nsExec::ExecToLog 'powershell -NoProfile -NonInteractive -Command "\
  $$dirs = @(\
    ...
    (Join-Path $$env:APPDATA \"Workhorse AI\openclaw\state\openclaw-weixin\accounts\")\
  );\
  foreach ($$d in $$dirs) {\
    if (Test-Path $$d) {\
...
其中 `%APPDATA%\Workhorse AI\openclaw\state\openclaw-weixin\accounts` 是 Workhorse AI 管理的 OpenClaw 状态目录下的微信登录凭据目录。覆盖安装删除它后，本地 SQLite 里仍然保存 `weixin.accountId`，但插件读取不到对应账号 token，于是报 `not configured`。
...
2. 凭据保存在 Workhorse AI 的 SQLite IM 配置中。
3. 启动或保存时，Workhorse AI 将这些配置同步到 OpenClaw config 或 secret env。
...
5. Workhorse AI 本地 IM 配置只保存 `enabled`、`accountId`、`dmPolicy`、`allowFrom` 等控制配置，不保存真正的微信 token。
...
**Given** 用户已经在 Workhorse AI 中扫码连接微信  
**When** 用户安装新版本覆盖旧版本  
**Then** 微信登录态必须保留，打开 IM 设置时不应显示 `not configured`。

### 场景 B: 覆盖安装后启动 OpenClaw gateway

**Given** 用户的 `%APPDATA%\Workhorse AI\openclaw\state\openclaw-weixin\accounts` 中存在微信 token 文件  
**When** 覆盖安装完成并启动 Workhorse AI  
...
```text
%APPDATA%\Workhorse AI\openclaw\state\openclaw-weixin\accounts
```
...
Workhorse AI 管理的 OpenClaw runtime 启动时会显式设置：

```text
OPENCLAW_STATE_DIR=%APPDATA%\Workhorse AI\openclaw\state
```

因此 Workhorse AI 的微信登录态边界应限定在：
...
该目录属于 standalone OpenClaw 的默认状态目录。Workhorse AI 读取它可能误用用户另一个 OpenClaw 实例的微信账号，造成账号串用、状态污染或安全边界不清。

当前 installer 中对 `%USERPROFILE%\.openclaw\openclaw-weixin\accounts` 的删除也应移除。移除理由不是为了迁移该目录，而是为了避免 Workhorse AI installer 破坏 standalone OpenClaw 的用户数据。
...
2. 对 `%APPDATA%\Workhorse AI\openclaw\state\openclaw-weixin\accounts` 的 `Remove-Item`。
...
1. 该目录不是 Workhorse AI 管理的状态目录。
2. 用户可能同时安装 standalone OpenClaw，里面的微信 token 不一定属于 Workhorse AI。
3. 自动复制 token 会让两个运行环境共享或串用同一微信登录态。
4. 对已经被旧 installer 删除的 Workhorse AI token，如果 app-managed 目录中没有备份，无法安全自动恢复。
...
- `src/main/libs/openclawEngineManager.ts`：提供 Workhorse AI app-managed OpenClaw stateDir，明确状态目录边界。
...
2. 打开 IM 设置，扫码连接微信。
3. 确认 `%APPDATA%\Workhorse AI\openclaw\state\openclaw-weixin\accounts` 下存在账号 token 文件。
4. 覆盖安装修复后的版本。
5. 启动 Workhorse AI。
6. 打开“设置 → IM 机器人 → 微信”。
