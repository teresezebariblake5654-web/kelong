...
用户在 Cowork 会话中发起一次较长任务后，Workhorse AI 界面展示系统提示：
...
- UI 误以为模型已经空回复结束。
- 后续真实 assistant 文本没有进入 Workhorse AI 会话。
- OpenClaw session jsonl 中可以看到完整最终回复。

本问题不是模型 API 的真实空输出，而是 Workhorse AI 对 OpenClaw `chat.final`、上下文压缩重试和 run 生命周期的衔接判断过早。
...
5. Workhorse AI 已经把当前 run 标记为 completed 并清理 active turn。
...
| 字段 | 值 |
|---|---|
| Workhorse AI sessionId | `63592da9-09d9-40e9-8927-daf0b94e46e1` |
...
3. Workhorse AI 在 retry 生成可见文本前收到了 `chat.final` 或 final history 同步结果。
...
10. Workhorse AI 因 run 已进入 recently closed 集合，丢弃这些后续文本。
...
**Then** Workhorse AI 应保持当前会话 running 或 maintenance 状态  
**And** 不应提前展示 `[模型未输出内容]`  
...
**When** Workhorse AI 收到 context maintenance / compaction 相关事件  
...
**When** Workhorse AI 收到该 runId 的 assistant stream 或 final  
...
**When** run 真实结束  
**Then** Workhorse AI 可以展示 `[模型未输出内容]`

### 场景 5: 真实 late event

**Given** 某个 run 已正常完成且没有压缩 retry 延续标记  
**When** 后续收到旧 run 的过期 assistant 事件  
**Then** Workhorse AI 仍应丢弃该事件，避免污染新的会话状态
...
当 OpenClaw 进入上下文压缩、memory flush、silent maintenance 或压缩 retry 时，Workhorse AI 应向 renderer 发出明确的 maintenance active 状态。
