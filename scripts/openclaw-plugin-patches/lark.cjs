...
// Lightweight setup entry for deferred loading (patched by Workhorse AI).
...
function patchFilenameEncoding(larkPluginDir, log) {
  ...
  const replacement = `fileName = decodeURIComponent(match[1].trim());
                // Patched by Workhorse AI: fix Latin-1 garbled UTF-8 filenames from Feishu API
  ...
  if (fnIdx !== -1) {
    const helperFn = `// Patched by Workhorse AI: detect and fix Latin-1 garbled UTF-8 filenames.
