 ...
 * 通过 Workhorse AI.exe (ELECTRON_RUN_AS_NODE=1 模式) 执行。
 *
 * 用法: Workhorse AI.exe <本脚本路径> <tarPath> <destDir>
 *
 * 效果:
...
if (!tarPath || !destDir) {
  console.error('[unpack-cfmind] Usage: Workhorse AI.exe unpack-cfmind.cjs <tarPath> <destDir>');
  process.exit(1);
}
