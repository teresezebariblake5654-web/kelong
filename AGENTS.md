...
This root `AGENTS.md` is repository-level guidance for Workhorse AI. Codex may also
...
Workhorse AI is an Electron + React desktop application. Its core user-facing
...
`Cowork` is Workhorse AI's product/session layer. The name is historical: it
started as a Claude Code-like in-house coding assistant, but in the current
codebase it means the Workhorse AI layer that owns sessions, messages,
...
When changing OpenClaw-related behavior, first look for a Workhorse AI-side
...
Workhorse AI when the behavior is product-specific or can be expressed cleanly at
the integration boundary.

Use version-scoped OpenClaw patches only when the required behavior is inside
OpenClaw and there is no clean Workhorse AI-side hook. Do not avoid a patch by
adding brittle or contorted Workhorse AI workarounds.
...
- `src/main/libs/openclawConfigSync.ts`: renders Workhorse AI state into
...
Important paths:
- `%APPDATA%/Workhorse AI/openclaw/state/openclaw.json` on Windows: generated
  OpenClaw config.
- `%APPDATA%/Workhorse AI/openclaw/state/workspace-main`: main agent workspace.
- `%APPDATA%/Workhorse AI/openclaw/state/workspace-{agentId}`: non-main agent
  workspaces.
...
- `AGENTS.md`: OpenClaw workspace instructions with a Workhorse AI-managed section.
...
Main logs:
- Windows: `%APPDATA%/Workhorse AI/logs/main-YYYY-MM-DD.log`
- macOS: `~/Library/Logs/Workhorse AI/main-YYYY-MM-DD.log`
- Linux: `~/.config/Workhorse AI/logs/main-YYYY-MM-DD.log`

Main log retention is 7 days. Max file size is 80 MB; overflow rotates to
`.old.log`.

OpenClaw gateway capture logs:
- Windows: `%APPDATA%/Workhorse AI/openclaw/logs/gateway-YYYY-MM-DD.log`
- Retention is 3 days.
