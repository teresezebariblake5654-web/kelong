...
- `https://api-test.bx-aigc.com/s/{shareId}/`
...
The dedicated host lets user apps keep root-relative API paths and cookies scoped to their own share host. This avoids collisions with Workhorse AI server APIs such as `/api/*`.
