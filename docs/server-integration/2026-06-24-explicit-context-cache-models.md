...
- `qwen3.5-plus` and `qwen3.6-plus` are available through OpenAI-compatible Chat Completions for both `Workhorse AI` and `YoudaoInner` providers.
...
- `/api/proxy/chat/completions` does not inject cache markers; Workhorse AI/OpenClaw must send `cache_control` in the upstream payload when explicit cache is enabled.
...
- If server model metadata is not loaded yet, still infer these defaults for `qwen3.5*`, `qwen3.6*`, and `claude-*` Workhorse AI server model IDs.
