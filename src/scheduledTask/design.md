# Workhorse AI 定时任务系统设计文档

## 总述

Workhorse AI 的定时任务系统是一套横跨 **Renderer(UI) -> Main Process(IPC/业务逻辑) -> OpenClaw Gateway(调度引擎)** 三层的端到端自动化执行框架。它允许用户通过 UI 界面、IM 聊天或 Cowork 会话创建定时任务，由 OpenClaw 的 Cron 引擎进行调度触发，并将执行结果通过 IM 通道或 Webhook 投递给用户。

**核心设计理念**：

1. **OpenClaw 驱动** -- 所有定时任务的调度、执行、投递均由 OpenClaw Gateway 原生完成，Workhorse AI 作为上层应用只负责任务的 CRUD 和 UI 展示，不接管消息投递逻辑
...
Workhorse AI 的 `imStore` 中存储的 `conversationId` 带有 IM 子类型前缀（如 `direct:ou_xxx` 表示飞书私聊，`group:oc_xxx` 表示飞书群聊）。但 OpenClaw 的飞书插件中 `normalizeFeishuTarget()` 不识别 `direct:` 前缀，会将 `direct:ou_xxx` 原样传递给飞书 API，导致 400 错误。因此在传递给 OpenClaw 之前需要剥离此前缀。
...
`CronJobService` 是 Workhorse AI 与 OpenClaw Gateway 之间的 **适配器层**，封装了所有 Cron RPC 调用。
...
CronJobService 在 Workhorse AI 前端类型和 OpenClaw Gateway 类型之间进行双向转换：
...
OpenClaw 使用 `sessionKey` 来标识和管理会话。Workhorse AI 定义了三种格式：
...
Workhorse AI 的定时任务系统通过三层架构实现了完整的自动化执行能力：
...
3. **引擎层（OpenClaw Gateway）**：承担所有调度、执行、投递的核心工作，确保 Workhorse AI 不需要自行实现消息投递逻辑
...
| 前缀剥离而非修改 OpenClaw | Workhorse AI 的 `imStore` 需要带前缀的 ID 做内部路由，但 OpenClaw 的 Feishu 插件不识别该前缀，因此在传递给 OpenClaw 前剥离 |
