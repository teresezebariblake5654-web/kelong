/**
 * Where the failing model comes from in Workhorse AI terms, so users can tell at
 * a glance whether an error concerns the Workhorse AI plan, a vendor coding plan
 * they configured, or their own custom provider.
 */
export const CoworkErrorModelSource = {
  WorkhorseAIPlan: 'lobsterai-plan',
...
export interface CoworkErrorDetail {
  /** Runtime error message before Workhorse AI i18n normalization. */
  rawErrorMessage?: string;
  provider?: string;
  model?: string;
  /** Workhorse AI-side classification of where the failing model comes from. */
