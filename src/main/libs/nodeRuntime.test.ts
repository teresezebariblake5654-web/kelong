...
describe('selectSpawnableNodeCandidate', () => {
  test('skips Workhorse AI bash and cmd shims on Windows and returns node.exe', () => {
    expect(selectSpawnableNodeCandidate([
      'C:\\Users\\demo\\AppData\\Roaming\\Workhorse AI\\cowork\\bin\\node',
      'C:\\Users\\demo\\AppData\\Roaming\\Workhorse AI\\cowork\\bin\\node.cmd',
      'C:\\Program Files\\nodejs\\node.exe',
    ], 'win32', 'C:\\Users\\demo\\AppData\\Roaming\\Workhorse AI')).toBe(
      'C:\\Program Files\\nodejs\\node.exe',
    );
  });
  ...
  test('returns null on Windows when only non-native shims are available', () => {
    expect(selectSpawnableNodeCandidate([
      'C:\\Users\\demo\\AppData\\Roaming\\Workhorse AI\\cowork\\bin\\node',
      'C:\\Users\\demo\\AppData\\Roaming\\Workhorse AI\\cowork\\bin\\node.cmd',
    ], 'win32', 'C:\\Users\\demo\\AppData\\Roaming\\Workhorse AI')).toBeNull();
  });
...
describe('resolveNodeRuntimeForSpawn', () => {
  test('uses real node.exe on Windows when present after Workhorse AI shims', () => {
    setPlatform('win32');
