...
const renderCallbackHtmlWithRedirect = (
  ...
  const redirectHint = redirectUrl
    ? '<p class="hint">页面将自动返回 Workhorse AI 登录页。</p>'
    : '';
  const redirectAction = redirectUrl
    ? `<a class="action" href="${escapeHtml(redirectUrl)}">立即返回</a>`
    : '';
  return `<!doctype html>
<html lang="zh-CN"><head><meta charset="utf-8"><title>Workhorse AI 登录</title>
<style>
...
async function createAuthLocalCallback(
  ...
  server.on('request', (req, res) => {
    if (req.method !== 'GET') {
      sendHtml(res, 405, false, '登录回调请求方法不支持，请返回 Workhorse AI 后重试。');
      return;
    }
    ...
    if (!code) {
      ...
      sendHtml(res, 400, false, '登录回调缺少授权码，请返回 Workhorse AI 后重试。');
      void callback.close();
      return;
    }
    ...
    if (returnedState !== state) {
      ...
      sendHtml(res, 400, false, '登录状态校验失败，请返回 Workhorse AI 后重试。');
      void callback.close();
      return;
    }
    ...
    try {
      ...
      sendHtmlWithRedirect(
        ...
        200,
        true,
        '登录已完成，正在返回 Workhorse AI 登录页。',
        returnTo,
      );
    } catch (error) {
      console.error('[AuthLocalCallback] failed to deliver auth code:', error);
      sendHtml(res, 500, false, '登录回调处理失败，请返回 Workhorse AI 后重试。');
    } finally {
      void callback.close();
    }
  });
