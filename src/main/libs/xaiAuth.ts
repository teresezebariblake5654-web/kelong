 ...
 * auto-refreshes it via the xai plugin's refreshOAuth hook — Workhorse AI never
...
async function persistXaiCredential(params: {
  ...
  await withAuthStoreLock(() => {
    ...
    // Workhorse AI models a single xAI account: drop any previous xai profiles so
...
function renderCallbackHtml(success: boolean, message: string): string {
  ...
  return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><title>Workhorse AI · xAI Login</title>
<style>
...
function waitForCallback(expectedState: string): Promise<{ code: string }> {
  return new Promise((resolve, reject) => {
    ...
    const server = http.createServer((req, res) => {
      ...
      res.end(renderCallbackHtml(true, 'You can now close this tab and return to Workhorse AI.'));
