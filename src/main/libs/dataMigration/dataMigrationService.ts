...
export const inspectMigrationArchiveSync = (
  ...
  if (!state.root || state.entryCount <= 0) {
    throw new Error('Backup archive is empty or missing Workhorse AI user data.');
  }
...
const extractMigrationArchiveToTempSync = (
  ...
  try {
    ...
    if (!fs.existsSync(sourceRoot) || !fs.statSync(sourceRoot).isDirectory()) {
      throw new Error('Backup archive did not extract a valid Workhorse AI user data directory.');
    }
