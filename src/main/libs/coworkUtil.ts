...
function resolveWindowsGitBashPath(): string | null {
  ...
  // 2. Bundled PortableGit (preferred default in Workhorse AI package)
...
function ensureWindowsBashUtf8InitScript(): string | null {
  try {
    ...
    const content = [
      '#!/usr/bin/env bash',
      '# Auto-generated by Workhorse AI – switch Windows console code page to UTF-8',
...
export async function probeCoworkModelReadiness(
  ...
  // OpenAI-compatible providers (e.g. 1701.store / custom_0) must be probed
  // against the upstream URL with Bearer auth. resolveCurrentApiConfig() routes
  // them through the local Anthropic-compat proxy, and probing that proxy with
  // the upstream key produces a false "invalid or missing proxy token" 401.
  const rawResolution = resolveRawApiConfig();
  if (rawResolution.config?.apiType === 'openai') {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), timeoutMs);
    try {
      const response = await fetch(buildOpenAIChatCompletionsURL(rawResolution.config.baseURL), {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${rawResolution.config.apiKey}`,
        },
        body: JSON.stringify({
          model: rawResolution.config.model,
          max_tokens: 1,
