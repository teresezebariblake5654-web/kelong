...
const INSTALLER_PATH = 'C:\\Users\\test\\AppData\\Roaming\\Workhorse AI\\updates\\lobsterai-update-manual-1.exe';
...
describe('hdiutil plist parsing', () => {
  test('extracts the mount point and dev entries from an APFS attach result', () => {
    ...
    const json = JSON.stringify({
      'system-entities': [
        ...
        { 'dev-entry': '/dev/disk5s1', 'mount-point': '/Volumes/Workhorse AI', 'volume-kind': 'apfs' },
    ...
    expect(result.mountPoint).toBe('/Volumes/Workhorse AI');
  ...
  test('extracts the mount point from an HFS attach result', () => {
    const json = JSON.stringify({
      'system-entities': [
        { 'content-hint': 'GUID_partition_scheme', 'dev-entry': '/dev/disk4' },
        { 'content-hint': 'Apple_HFS', 'dev-entry': '/dev/disk4s1', 'mount-point': '/Volumes/Workhorse AI 1' },
      ],
    });

    const result = parseHdiutilAttachOutput(json);

    expect(result.mountPoint).toBe('/Volumes/Workhorse AI 1');
  });
...
describe('mac swap builders', () => {
  test('places staging and backup next to the target app, hidden and not .app-suffixed', () => {
    const swapPaths = buildMacSwapPaths('/Applications/Workhorse AI.app', 1234);

    expect(path.dirname(swapPaths.staging)).toBe('/Applications');
    expect(path.dirname(swapPaths.backup)).toBe('/Applications');
    expect(path.basename(swapPaths.staging)).toBe(`.Workhorse AI.app${MAC_SWAP_STAGING_INFIX}1234`);
    expect(path.basename(swapPaths.backup)).toBe(`.Workhorse AI.app${MAC_SWAP_BACKUP_INFIX}1234`);
  ...
  test('builds a staged-copy, guarded-backup, rollback and cleanup sequence', () => {
    const target = '/Applications/Workhorse AI.app';
    const swapPaths = buildMacSwapPaths(target, 7);

    const cmd = buildMacSwapInstallCommand('/Volumes/Workhorse AI/Workhorse AI.app', target, swapPaths);

    const cpIndex = cmd.indexOf(`cp -R '/Volumes/Workhorse AI/Workhorse AI.app' '${swapPaths.staging}'`);
...
describe('macOS DMG install', () => {
  ...
  const USER_DATA = '/Users/test/Library/Application Support/Workhorse AI';
  const DMG_PATH = `${USER_DATA}/updates/lobsterai-update-auto-1.dmg`;
  const TARGET_APP = '/Applications/Workhorse AI.app';
  ...
  const respondMountedAtVolumes = () => attachMountedJson('/Volumes/Workhorse AI');
  ...
  beforeEach(() => {
    ...
    execCommands = [];
    execOverride = null;
    applicationsEntries = ['Workhorse AI.app'];
  ...
  test('mounts, swap-installs and relaunches on the happy path', async () => {
    ...
    expect(detachCommands[0]).toContain('/Volumes/Workhorse AI');
  ...
  test('cleans up leftover staging and backup directories before installing', async () => {
    attachResponders = [respondMountedAtVolumes];
    applicationsEntries = [
      `.Workhorse AI.app${MAC_SWAP_STAGING_INFIX}1`,
      `.Workhorse AI.app${MAC_SWAP_BACKUP_INFIX}2`,
      'Workhorse AI.app',
      'Other.app',
    ];
    ...
    expect(fs.promises.rm).toHaveBeenCalledWith(
      `/Applications/.Workhorse AI.app${MAC_SWAP_STAGING_INFIX}1`,
      { recursive: true, force: true },
    );
    expect(fs.promises.rm).toHaveBeenCalledWith(
      `/Applications/.Workhorse AI.app${MAC_SWAP_BACKUP_INFIX}2`,
      { recursive: true, force: true },
    );
