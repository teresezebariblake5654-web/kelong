 ...
 * and waits for the user's response via an HTTP callback to Workhorse AI.
 *
 * This enables delete-confirmation modals on the Workhorse AI desktop app
...
const plugin = {
  ...
  description: 'Structured user confirmation tool for Workhorse AI desktop.',
  configSchema: {
  ...
  register(api: OpenClawPluginApi) {
    ...
    // Use a factory so the tool is only available for Workhorse AI local-session candidates.
    // IM channel sessions (qqbot, dingtalk, weixin, feishu, etc.) get null → tool hidden.
    api.registerTool((ctx) => {
      // Enable for Workhorse AI desktop sessions across agents and delegated child sessions.
